...
最後更新：2026-09-03（**第 45 批：通知紀錄不再洗版「時程變更軌跡」**
——使用者問「若有人寄很多封，歷史軌跡的顯示是否會太雜亂?」。實測 7 筆通知：軌跡總高
365px → **951px**，通知獨佔 586px（62%），而面板可視高度只有 224px。
我提了三個方案，他選方案 1：**`通知寄送` 抽出時間軸，收成面板上方一行摘要**
（`✉ 已通知 N 次 · 最後 <時間> · <誰> → <收件者>` + 展開）。實測收合 **47px**、展開 **177px**
（每列 22px）。⚠️ 第 35 批的 `changeGroups` 對通知**完全沒用**（合併條件含時間與說明，
而每次通知時間必然不同）。⚠️ 精簡的是**顯示**不是**紀錄**：稽核列一筆沒少、完整 Note 掛在
每行的 title，「未確認送出」用紅字顯示在畫面上（不可收進 tooltip）。
新增 `hasTimeline`（不可再用 `hasHist` —— 「只有通知、沒有時程變更」會畫出空白捲動區）。
**沒有動 `Program.cs`、沒有 SQL 腳本。** `?v=20260903001`。手冊第 10 章已同步。
⚠️ 同一次對話中使用者也明確表示 **D／E／F 都不做**：
D（副本與收件者同一人去重）**刻意不做** —— 他的原話「EMS 跟 MSD 人員不會是同一個，
除非是我要測試用，那我還是需要看到寄件者跟附件收件者」；
E（限制寄幾封）不做 —— 「手動寄信不用限制使用者要寄幾封，但寄信需要留下歷史紀錄」；
F（`Mail:AppUrl`、前端不知道 `mailReady`）不做。
測試資料（12 筆 `通知寄送`，分兩批）已全部刪除並複驗，NID 6 原本的 5 筆軌跡完好。

前一批是 2026-09-02（**第 44 批：寄信邏輯檢查後修 A＋B**（使用者要求「重新檢查寄送信件的邏輯」，
我出了 A~F 六項清單，他挑 A＋B）。
**A：`SmtpClient.Timeout` 對 `SendMailAsync` 完全無效** —— 實測 Timeout 設 3 秒、對「接受 TCP 連線
之後不回話」的假 relay 寄信，**60 秒後仍未返回**。第 40 批那個 TcpClient 探測攔不到（它只管
「連線建立」）。後果是前端 fetch 無限等 + `runExclusive` 鎖死整個分頁。改用
`SendMailAsync(msg, cts.Token)`（實測 3.05 秒中斷，且真的取消底層作業，`.WaitAsync` 不會），
前端補 90 秒 AbortController 保險絲。**逾時走 dbmail 那條「未確認送出」的路，不判成失敗**
（信可能已經送出，判失敗會讓使用者再按一次而對方收到兩封）。
**B：信箱格式在端點層先驗**（`IsValidMailAddress`）—— `dbo.Assignee.EMAIL` 是手動維護的，
打錯時原本會冒出一句英文 + 502。順便解掉 C（`a@x.com;b@y.com` 在 smtp 被擋、在 dbmail 會
**真的寄給兩個人**）。⚠️ 驗證器**必須接受沒有點的網域**（`Chih_Kuan_Chang@UMCG`）與 Outlook 的
「姓名 <位址>」，兩者實測 .NET 都收 —— 前端 regex 一度比後端嚴，已修正並逐項對齊（18 case）。
**有動 `Program.cs`**（`SendNotifyMailAsync` 的簽章改成回 tuple）、沒有 SQL 腳本。`?v=20260902005`。
⚠️⚠️ **這批動到了在 IIS 實測過的寄信路徑，必須回 p58esiap12 重測一次再視為完成。**
本機端到端驗過：假 relay 停頓 → 6.4 秒返回 + 稽核列標「未確認送出」；四種壞信箱 → 0.1 秒回 400
且訊息指名是誰的信箱、去哪裡改（測試暫改 dbo.Assignee 第 7 筆，已還原並複驗）。
未做的：D 副本與收件者同一人會收兩份、E 兩人同時按會寄兩封、F（`Mail:AppUrl` 空的／前端不知道
`mailReady`）—— 使用者只挑了 A＋B。

前一批是 2026-09-02（**第 43 批：同一階段的未壓日期通知，存檔後只自動問一次**
——使用者要求「只要同一個狀態寄信一次成功、我就不強迫每次儲存都會跳出視窗詢問，
只寄信一次盡到通知的責任」。新增 `phaseNotifiedEntry()`（`app.jsx`），判定鍵是
**(需求, 階段, 收件者信箱)**、基準線是同一階段最後一次 `規格回退` 之後、依據是稽核列的
`通知寄送`（**不是 localStorage**）。**負責人換人一定要重問**（使用者原話：「變更的負責人
沒收過信件，完全不知道有這件事。我一定要盡到有通知的責任」）—— 比對稽核 `Note` 裡
`收件者 … <email>` 的信箱，判不出來時一律當成「還沒通知」往安全方向倒。`未確認送出` 不算寄過。
順手讓 `fetchHistory()` 回傳剛抓到的那一份（存檔後那條路不能讀 state，`setState` 非同步）。
手動 ✉ 完全不擋，只多一行「已經在 X 通知過了，這會是第二封」。
**沒有動 `Program.cs`、沒有 SQL 腳本** —— 這批只改前端的觸發條件，
**不碰第 41 批那條已在 IIS 實測通過的寄信路徑，不需要回 IIS 重測寄信**。
`?v=20260902003`。手冊第 05／10 章已同步。
邏輯用 14 個 case 驗過（含那條解析 `Note` 的 regex 對上 `Program.cs:2319` 的實際格式）。

前一批是 2026-09-02（**第 42 批：新增需求時 `1_EMSEnd` 由必填改為選填**
——使用者要求「不強求註冊新需求的當下就要提供 ① 的結束日」。必填條件只剩「**原本就有值**」
（仍然擋得住手動清空既有的結束日）。**第二半（存檔後問要不要寄信、資料列的 `✉`）
第 39 批就做好了，這批沒有動它** —— 在此之前它只是永遠不會在「新增」這條路上被觸發，
因為 ① 一定有日期。**有動 `Program.cs`**（`MissingRequiredFields()` 一行條件），
沒有 SQL 腳本。`?v=20260902001`。順手改掉回退視窗那句從第 21 批起就不成立的
「起訖日是必填欄位，清空後必須重新填寫才能儲存」。
⚠️ **第 41 批的寄信設定（`Mail` 區塊、`smtp` 模式、relay 10.13.2.221）使用者已在公司 IIS 實測通過，
往後不要動那一段邏輯。**）

前一批是 2026-09-01（**第 41 批：送信方式改成可切換，使用者選擇走 DB 主機的 `sp_send_dbmail`**
...
⚠️ **`Mail:Host` 還是空的（使用者說他自己填），功能還沒真的寄出過一封信到 Notes**。
同一天先做了 **EMAIL 欄那一批**（`15_add_assignee_email.sql`，`?v=20260831001`），
使用者之後已自行把 13 個人的 `EMPO` 與 `EMAIL` 全部補齊）

前一批是 2026-08-27（**第 38 批：移除「延期完成 2 次以上」篩選選項**，併回「有延期完成」；
⚠️ 徽章的「2 次以上轉紅」刻意保留。
同一天先做了 **第 37 批：「執行延期」正名為「延期完成」**，用語一次對齊五處，
並移除 `FilterSelect` 的 per-option `hint`（那是破版的來源）。
同一天先做了 **第 36 批：修工具列破版** ——
原生 `<select>` 的寬度由最長的 option 撐出來，第 34 批加的說明把「警示」撐成 365px，
把動作鈕擠到第二行。**那是我自己造成的迴歸。**
同一天先做了 **第 35 批：軌跡收合（一次動作＝一張卡）＋ 新增 ChangeType `重新排程`** ——
回退後重壓的日期原本被判成 `init`，沉到「初始時程」區。**那批有動 `Program.cs`**，
並產出 **`14_fix_reschedule_changetype.sql`（已執行，修正 1 列）**。
同一天先做了 **第 34 批：「警示」下拉的「延期」補一句說明** ——
畫面上沒有叫「延期」的按鈕，它是「✓ 完成」的判定結果。
同一天做完 **第 33 批：新增「已到階段卻沒壓日期」＝逾期未壓**，標紅字並排在「逾期優先」最上面。
兩批都沒有 SQL 腳本、都沒有動任何 C# 檔。前一批是 2026-08-24 的第 32 批（投影模式的前置條件））
...
| 新增必填 | **`NID`、`MainCat`、`SubCat`、`EMS` 四欄**。`1_EMSStart` 於 2026-08-22 拿掉、**`1_EMSEnd` 於 2026-09-02（第 42 批）拿掉** —— 現在 `1_EMSEnd` 只在「原本就有值」時必填（擋手動清空）。留空的新需求會顯示「⚠ 未壓日期」＋`✉`，存檔後直接問要不要寄信通知 EMS 負責人 |
...
| 逾期未壓（2026-08-27 / 第 33 批） | **`unsetDuePhase()`**：`StatusID` 已經走到某一階段、那一階段自己卻沒有日期 → `level='unset'`，資料列那一格標「⚠ 未壓日期」、算進「需關注」、「逾期優先」排**最上面**。入口統一走 **`resolveFocusPhase()`**（先問未壓、沒有才退回 `resolveDuePhase()`）。⚠️ **不受 7 日窗限制**（它根本沒有日期可比）；⚠️ `StageCode` 空白或超出 1~5 的**一律不推斷**；⚠️ 判斷一律比 `e.level`，**不可以拿 `diffDays` 反推**（`null <= 7` 在 JS 裡是 **true**、`null < 0` 是 false） |

---

## 待辦（下次接手從這裡開始）

### 🟦 目前環境狀態（2026-08-31 / 第 39 批實測，取代下面那張 2026-08-23 的表）
...
| `?v=` | **`20260902002`**（第 42 批的第二次建置：加「有信箱才詢問」；`20260902001`＝第 42 批第一次、`20260901002`＝第 41 批、`20260901001`＝第 40 批、`20260831003`＝第 39 批） |
...
### 🟦 舊的環境狀態（2026-08-23 實際量測）

| 項目 | 值 |
|---|---|
| DB 資料 | `dbo.Controltable` **62 筆**（全部 `IsDeleted=0`，**已無軟刪除的列**——使用者後來重新匯入過）；`dbo.Controltable_History` **222 筆**；三個計數欄總和 **0**（第 23 批測完已完整還原並複驗） |
| `?v=` | **`20260831002`**（第 39 批；`20260831001`＝同一天稍早的 EMAIL 欄那批。以下是舊的：`20260827006`＝第 38 批、`005`＝第 37 批、`004`＝第 36 批、`003`＝第 35 批、`002`＝第 34 批、`001`＝第 33 批。`20260824014` 是第 32 批：002~005＝第 27 批、006＝28、007~009＝29、010~011＝30、012＝31，中途的建置不算不同版本） |
| 需關注 | **2026-08-27 起是 10 件（逾期仍是 3、未壓 0）**：`soon` 7 筆＝NID 6/10/20/30/31/35/36，`overdue` 3 筆＝NID 12(-7)/61(-6)/62(-6)。日期又往前滾了三天，NID 20（③ 08-28）與 35 進了 7 日窗。**全表沒有任何一筆「已到階段未壓日期」**（62 筆的 `StageCode` 是 1×4／2×3／3×7／4×3／5×45，每一筆當前階段都有日期），第 33 批因此是造暫時資料驗的。<br>（2026-08-24 的舊值是 9 件）**2026-08-24 起是 9 件（逾期仍是 3）**。⚠️ **這不是壞掉，是日期往前滾了一天**：預警窗是 `DUE_WINDOW_DEFAULT = 7` 天，NID 6 / 10 / 30 / 31 / 36 五筆的到期日剛好落在 `today+7`，08-23 當天還在窗外（+8）、08-24 就進來了。逾期那 3 筆從頭到尾沒變：NID 12（-4）、61（-3）、62（-3）；NID 20 是 +4（③ 08-28）。**下次數字又不一樣時先用 `buildDueList(data, DUE_WINDOW_DEFAULT)` 印出 `diffDays` 再判斷**，別急著當成 bug。<br>（2026-08-23 的舊值是 4 件／逾期 3。第 23、24 批兩次動到逾期判定，**新舊規則在這 62 筆上結果都完全相同**；第 25、27 批沒有動判定邏輯） |
...
### ✅ 第 42 批（新增需求時 `1_EMSEnd` 改為選填）—— **完成，無 SQL 腳本**（2026-09-02）

使用者的原話：**「將 1_EMS規格確認的 End Date 從原本的必填改為選填，不強求使用者在註冊新需求
的就要立馬提供」**，後半段是「儲存時若目前階段未壓日期，要跳出是否寄信通知負責人的視窗、
以及在畫面上顯示寄信的 icon」。

⚠️ **後半段第 39 批就做好了，這批只在它前面多加一個條件（收件者要有信箱），其餘一行都沒動。**
`handleSave()` 的觸發條件本來就是「存檔成功 + `unsetDuePhase(fresh)`」，
資料列的 `✉`（`NotifyMailButton`）也早就掛在「⚠ 未壓日期」徽章下面 ——
**它只是永遠不會在「新增」這條路上被觸發**，因為 ① 一定有日期。
必填一放寬，那兩個出口就自己接上了。**下次遇到「新功能」先查一次現有程式**：
這批真正的改動只有一個布林條件。
...
| 檔案 | 改動 |
|---|---|
| `Program.cs` | `MissingRequiredFields()`：`before == null \|\| 原本有值` → **`before != null && 原本有值`**。一行 |
| `app.jsx` | `requiredFieldsFor()`：`!orig \|\| isDateVal(...)` → **`orig && isDateVal(...)`**（前後端同一套） |
| `app.jsx` | 新增 `specEndRequired`，① 的 End 標籤在必填時才有紅星、否則標「(可不填)」並在底下說明留空的後果 |
| `app.jsx` | 回退視窗那句「起訖日是必填欄位，清空後必須重新填寫才能儲存」改掉 —— **它從第 21 批起就已經不成立**，留著會讓人不敢按回退 |
| `app.jsx` | `handleSave()` 存檔後的自動詢問**多一個條件：`notifyPreview(fresh).problem` 為空**（使用者追加要求：「有信箱時才詢問。我一定會補齊信箱」） |
| `CLAUDE.md` | 新增兩條：寄信路徑「已於 IIS 實測通過，未經指示勿改」（使用者要求寫進 repo），以及上面那條自動詢問的界線 |
| `FIELD_SPEC.md` / `使用者手冊.html` | 情況一的必填清單、第 03 章的步驟與新的說明框、欄位對照表 |

⚠️ **紅星不可寫死**（`specEndRequired`）：標了紅星卻存得進去、或沒標紅星卻被擋下來，
兩種都是使用者無從理解的。它與 `requiredFieldsFor()` 走同一條界線。

⚠️ **「(可不填)」底下那句話是必要的**，不是裝飾：① 的 End 是**唯一一個留空就會讓這筆需求
一存檔立刻掛上紅色警示**的欄位。只寫「可不填」會被讀成「這格不重要」，
使用者反而可能為了避開警示先隨手填一個假日期 —— 那正是這批要消滅的東西
（假日期會流進逾期判定與統計，比沒有日期更難查）。

**為什麼必填條件要留「原本就有值」而不是整條拿掉**：那條是用來擋**手動把既有的結束日清空**的。
系統自己清掉的（`/rollback` 到 ①）不擋，見第 21 批。

**實測**（本機 `Sariel` DB，造 NID 9901 再軟刪除還原）：

1. 新增視窗的 ① End 標籤 = `End Date (可不填)`、底下出現那句說明 ✅
2. 只填 NID／Main Cat／Sub Cat／EMS 負責人就存得進去（`POST /api/requirements` → 200）✅
3. 存檔成功後**立刻跳出**「寄信通知壓定日期」視窗，內文正確指向「① EMS規格確認」，
   收件者靖凱、副本玉婷 ✅（按取消，**沒有真的寄信**）
4. 資料列出現 `⚠ 未壓日期` + `✉`（`aria-label` = 「寄信通知負責人壓定「1_EMS規格確認」的日期」），
   「已到階段未壓日期」由 2 → 3 ✅
5. 開既有的 NID 2（① 有日期）→ 標籤是 `End Date *`、沒有那句說明 ✅
6. 軟刪除測試資料後計數回到 2，`DELETE` → 200 ✅

**追加：「有信箱時才詢問」的實測**（暫時把靖凱的 `EMAIL` 設成 `NULL`，測完已還原並複驗 13 筆全有信箱）：

- 沒信箱 → 存檔成功、**沒有跳任何視窗**，但那一列的 `⚠ 未壓日期` 與 `✉` 都在 ✅
- 同一列按下 `✉` → 仍然跳「無法寄出通知：指派人員主檔裡「靖凱／EMS」沒有填 EMAIL…請直接在 SSMS 補上」✅
- 還原信箱後再建一筆 → 詢問視窗照跳，收件者靖凱、副本「MSD 還沒指派負責人，這次不會有副本」✅

⚠️ **不要把 `✉` 也一起藏起來**。它是「這一列為什麼被算進需關注」的唯一出口，
而且使用者主動按下去時，系統一定要回答「寄了沒」。第 39 批那條界線只縮到「他主動要寄」的時候。

⚠️ 環境副作用：本機 `dbo.Assignee` 沒有登入者 `yu-tinglin` 的工號，
所以確認視窗的寄件者顯示「系統預設信箱」。**那是本機的資料問題，不是這批的**
（公司 IIS 上使用者的工號與信箱都補齊了）。
...
使用者附截圖（NID 20：`StatusID = 4 EMS驗收`、④ 那格是「⚠ 未壓日期」）要求：
**「在該階段的工作者完成自己的工作時，可以有自動寄信的方式告知已完成，請下一棒來壓日期。」**
兩個入口：①儲存後自動問 ②那一格裡的手動 `✉`。
⚠️ **這批有動 `Program.cs`**，跑過 `dotnet build`：0 警告 0 錯誤。**沒有動資料庫結構。**

**先問過使用者的兩件事（他的回答）**

| 問題 | 回答 |
|---|---|
| 寄信方式 | **「公司是用 Notes 寄信，不是 Outlook。可以幫我預設點選後寄出嗎?」** → 後端 SMTP 直寄，**不是 `mailto:` 開草稿** |
| 何時跳出 | **「儲存成功後，只要當前階段未壓日期就問」**（不是「只在階段被推進時才問」） |
| SMTP 主機 | **「可以留給我自己填，告知我去哪裡改 config 就好」** → `appsettings.json` 的 `Mail:Host`，**目前還是空的** |
| 寄件者信箱 | **他附了 `dbo.Assignee` 的截圖說「你幫我讀 DB 內的資料表如上圖」** —— 同一天他把 `EMPO`（工號）與 `EMAIL` 13 筆全部補齊了。⚠️ **`EMPO` 就是答案**：Windows 帳號剝掉網域就是工號，所以**寄件者＝按下按鈕的那個人本人** |
| 帳密／SSL | **「不需要」** |
| 系統網址 | **「設成可以 config 修改」** → `Mail:AppUrl` |

**做法**

- `POST /api/requirements/{id}/notify-unset`（body 只有 `actorEmpId` / `actorSource`）。
- **收件者＝那一階段的負責人，副本＝另一邊的負責人**（使用者定義）。
  ①④ 是 EMS、②③ 是 MSD（`StageSideOf()`，與 `DUE_PHASES.side` 一致）。
  信箱查 `dbo.Assignee` 的 `(DEPT, NAME)`，兩邊 trim、**不濾 `IsActive`**（與 `assigneeEmailOf()` 同一套）。
- ⚠️ **收件者、階段、主旨、內文全部由後端自己算**（`UnsetPhaseOf()` / `StagePassed()`，
  是 `app.jsx` `unsetDuePhase()` / `isPhasePassed()` 的**鏡像，改了要兩邊一起改**）。
  前端 `notifyPreview()` 只負責「先給使用者看一眼要寄給誰」。理由：這一支會**真的把信寄出去**，
  收件者若能由呼叫端指定，任何網頁都能借系統的名義寄信給指派名單上的人。
  同理也套了 `IsCrossSiteRequest()`（寄錯的信收不回來）。
- ⚠️ **先寄信、再寫稽核列**（`ChangeType='通知寄送'`）。反過來寄失敗就會留下一筆
  「已通知」的假紀錄而對方什麼都沒收到；**稽核列寫失敗時不可以回失敗**
  （信已經送出去了，使用者會再按一次而對方收到第二封）。
  `通知寄送` **不進 `isDateChange`**、不動三個計數欄。
- **寄件者＝按下按鈕的那個人本人**（`AssigneeByEmpNoAsync()`）：Windows 帳號 → `EMPO` → `EMAIL`。
  收件者因此可以**直接回信**問他，信的落款也跟著改口
  （本人 →「本信由「宸詳」透過需求管控表發出，有問題可以直接回覆本信」／
  系統信箱 →「請勿直接回覆本信箱」）。稽核列的 `Note` 也記寄件者。
  ⚠️⚠️ **只有 `actorSource == "windows"` 才用本人身分**。模擬帳號走這條路等於讓任何人挑一個名字、
  用他的身分把信寄出去 —— **那是真的冒名**，比稽核列標一個 `simulated` 嚴重得多。
  模擬／取不到帳號／工號不在名單上一律退回 `Mail:From`（**後備**，可留空），兩邊都沒有才回 400。
  ⚠️ `mailReady` **只看 `Host`**：拿不到寄件者是「那一次呼叫」的問題，不是整個功能沒開。
  ⚠️ 副本**刻意不因為「等於寄件者」而拿掉** —— 使用者舉的例子（宸詳做完 ③ 去通知明翰、cc 宸詳）
  裡，宸詳正是按鈕的人，他自己說了要 cc 給他。
- ⚠️ **寄不出去一律講清楚原因**：`Mail:Host` 沒設定（400）、負責人沒指派（400）、找不到寄件者（400）、
  `dbo.Assignee` 沒填 `EMAIL`（400，明講「那欄唯讀，請在 SSMS 補」）、SMTP 掛掉（502，
  把伺服器實際回的話一起帶出來）。**副本查不到信箱照樣寄給主要收件者**，只標 `ccMissing`。
- 郵件走內建 `System.Net.Mail`，**沒有引入新的 NuGet 套件**。設定在 `appsettings.json` 的 `Mail` 區塊。

**⚠️ `✉` 一定要疊在徽章「下一行」，不可以貼在右邊 —— 實測**

| 擺法 | 16 欄表格最小寬度 |
|---|---|
| 內嵌在徽章右邊 | 1254 → **1265px（多 11px）** |
| 疊在徽章下面 | 1254 → **1254px（0px）** |

欄寬是由「那一欄裡最寬的一格」撐出來的，而徽章（~70px）遠比按鈕（21px）寬，
疊下去等於 `max(徽章, 按鈕)` = 徽章寬。那一格本來就是最矮的一種
（沒有到期標籤、也沒有實際完成日那行），多一行剛好與旁邊逾期的格子一樣高，**列高不受影響**。
⚠️ 這條已經寫進 `CLAUDE.md`（與第 31 批的 1237px 預算是同一件事）。

> ⚠️ **memory.md 裡「16 欄壓縮到極限是 1237px」這個數字要改讀成 1254px** ——
> 不是這一批造成的：1237 是第 31 批量的，**當時全表沒有任何一列有「⚠ 未壓日期」徽章**，
> 而那顆徽章比一個日期字串寬。現在 NID 6 與 NID 20 都有，量到的就是 1254。

**驗證方式**（`dotnet build` 0/0；`npm run build`；`?v=` → `20260831003`）

- ⚠️ **`Mail:Host` 是空的，所以是用一個「落地成 .eml 檔」的暫時 patch 驗的**
  （`SmtpDeliveryMethod.SpecifiedPickupDirectory`，標 `TEMP-TEST-39`，**測完已完整移除**；
  暫時設定寫在 `appsettings.Development.json`，**也已還原**，`git diff` 確認該檔不在清單裡）。
  ⚠️ 試過在本機起一個 PowerShell 的 SMTP sink，**連不上**（沙箱裡 `TcpListener` bind 得起來，
  但另一個工具呼叫連 127.0.0.1 被拒）—— 別再走一次，直接用 pickup directory。
- **信件實測**（NID 20，`From: 宸詳 <Sw_Lee@UMCG>` / `To: Macgyver_Ho@UMCG` / `Cc: Sw_Lee@UMCG`，
  主旨 `[需求管控表] 請壓定「4_EMS驗收」日期：NID 20 / HBW / NonLotData`，
  `charset=utf-8` + base64，解出來的中文完全正確）—— **正是使用者舉的那個例子**。
- **寄件者五種情境全測過**：
  ① `00041856`(宸詳)/windows → `from=Sw_Lee@UMCG`、`fromIsSelf=true`、落款「可以直接回覆本信」
  ② `yu-tinglin`(不在名單)/windows → 退回 `Mail:From`、落款「請勿直接回覆」
  ③ **`00041856`/`simulated` → 沒有用本人身分**，一樣退回系統信箱（冒名擋住了）
  ④ `Mail:From` 留空 + 不在名單 → **400** 並講出工號與該去哪補
  ⑤ `Mail:From` 留空 + `00048599`(政翰) → 照常寄出（`fromIsSelf=true`）
  前端視窗兩種文案也都實測過（暫時在 `dbo.Assignee` 插一筆 `EMPO='yu-tinglin'` 驗「本人」那條，**已刪除**）。
- **前端**：`?due=unset` 兩列（NID 6 / 20）各一顆 `✉`；點 `✉` **不會展開那一列**
  （`stopPropagation`，實測 tbody 列數前後都是 2），視窗把收件者與副本的姓名＋信箱完整列出來。
  按「確認」→ toast「已寄出通知給 明翰」，展開明細後軌跡多一張「④ EMS驗收 / 通知寄送 / 時間 · yu-tinglin /
  說明：通知「4_EMS驗收」尚未壓定日期 → 收件者…副本…」，NID 欄**沒有多出 ⚠**。
- **儲存後自動問**：開 NID 20 的編輯視窗直接按「儲存變更」→ 跳出同一個視窗。
  **負面案例**：NID 2（stage 1、① 有日期）存檔 → **完全沒有跳任何視窗**，只有「已儲存變更」。
- **後端把關**：`id=2`（沒有未壓日期）→ 400「不需要通知」；`id=99999` → 404；
  `Sec-Fetch-Site: cross-site` → 400「偵測到跨站請求」；還原設定後 → 400「尚未設定郵件伺服器」。
- **版面**：1440 一般模式 overflow **0**、表格 1375px；1200 時表格 1254px（**加 `✉` 前後完全相同**）；
  精簡模式 9 欄 overflow **0**，那一欄也有 `✉`。console 無錯誤。
- **資料已完整還原**：測試寫進去的 `通知寄送` 稽核列與暫時的指派人員全部刪除 ——
  `leftover_notify = 0`、`leftover_tmp = 0`、`history_total = 232`、
  `assignee_total = 13`（**13 筆都有 EMAIL**）、`active_reqs = 62`。
  ⚠️ **NID 20 與 NID 2 的 `UpdatedAt` 停在測試當下的時間**（那兩次是空存檔，
  日期一欄都沒動、沒有產生任何稽核列），這不是壞掉。

**還沒完成的一件事**

**使用者說 SMTP 主機位址「留給我自己填」**，所以 `appsettings.json` 的 `Mail:Host` 目前是空的 ——
**功能到今天為止沒有真的寄出過一封信到 Notes**。他要填的只有 `Mail:Host`（Domino relay 位址）
與 `Mail:AppUrl`（信裡那行網址）兩個，其餘留預設即可（**已確認不需要帳密與 SSL**，
`Mail:From` 是後備、可留空）。⚠️ 下次接手時先確認他填了沒 —— 沒填的話任何寄信測試都會停在
「尚未設定郵件伺服器」，那**不是壞掉**。

### ✅ 第 38 批（移除「延期完成 2 次以上」篩選）—— **完成，無 SQL 腳本**（2026-08-27）

使用者：**「延期 2 次以上不用特別警示，幫我從選項中移除，統一併在有延期完成的選項就好」**。

`delay2` 六處全部移除：`ALERT_FILTER_LABEL`、`urlOne` 白名單、`matchAlertFilter`
的分支、`alertCounts`、下拉選項。警示下拉剩三個：
`📝 有時程異動` / `⏰ 有延期完成` / `🔄 有規格回退`。

⚠️ **只移除「篩選選項」，`⏰` 徽章「2 次以上轉紅」的行為刻意保留** ——
使用者講的是「從選項中移除」，而徽章那個是資料列上的視覺分級、不是篩選層級。
（已在回覆裡問他要不要一起拿掉，沒收到指示前不要自己動。）

⚠️ **舊網址 `?alert=delay2` 過不了白名單 → 退回「不限警示」並從網址上清掉**
（第 28 批那條「認不得的值一律退回預設」）。**刻意不做 `delay2 → delay` 的相容對應**：
那個值只活了幾個小時、而且 DB 裡 `DelayCount >= 2` 是 0 筆，
留一個永久的別名去救一個不存在的書籤不划算。

**驗證方式**（`npm run build`；`?v=` → `20260827006`；**沒有動資料庫、沒有動 C#**）

- 下拉剩 `不限警示` + 三個選項，`延期完成 2 次以上` 不見了。
- `?alert=delay2` → 收合顯示「不限警示」、**網址被清成空**、62 筆、無晶片。
- `?alert=delay` 照常 → **NID 6**（列上 `⏰1`）。
- 版面沒退回去（1440）：工具列高 **60px**、單行。console 無錯誤。

### ✅ 第 37 批（「執行延期」正名為「延期完成」）—— **完成，無 SQL 腳本**（2026-08-27）

使用者附截圖：**「第二個描述好像怪怪的，可以再修正嗎？」**
指的是第 36 批留下的 `⏰ 有執行延期 (1)　＝逾期後才按完成`。

**他是對的，而且問題不在字數。** 兩個毛病：
1. `按完成` 沒有引號時不成詞，整句是個讀不順的殘句
2. **只有這一個選項有尾巴**，看起來像沒清乾淨的殘骸

**正解是把名稱改對，不是繼續修尾巴。** 改用 `延期完成` ——
那是**稽核表實際寫入的 `ChangeType`**（見 `CHANGE_TYPES`），而且**詞裡就含著按鈕名「完成」**：
使用者當初問的「沒有延期功能為什麼有延期選項」，這個詞本身就是答案，補述整句可以拿掉。

用語一次對齊五處（**同一組字，不再各寫各的**）：

| 位置 | 舊 | 新 |
|---|---|---|
| 警示下拉 | `⏰ 有執行延期` / `⏰ 延期 2 次以上` | `⏰ 有延期完成` / `⏰ 延期完成 2 次以上` |
| 條件晶片 `ALERT_FILTER_LABEL` | `有執行延期` / `延期 2 次以上` | 同上 |
| 圖例列 | `⏰ 執行延期次數` | `⏰ 延期完成次數` |
| 資料列 ⏰ 徽章 tooltip | `執行延期 N 次` | `延期完成 N 次（按下「✓ 完成」時已超過原訂結束日）` |
| 排序面板「延期最多」tooltip | `依執行延期次數` | `依延期完成次數` |

完整說明仍在兩處：`<select>` 自己的 `title`、圖例列那顆的 tooltip。

**順手移除 `FilterSelect` 的 per-option `hint`**（第 34 批加的），已經沒有人用。
它正是第 36 批那場破版的來源。⚠️ **這條規則寫進註解與 `CLAUDE.md` 了：
不要再把說明接在 option 的文字後面**，兩個實測理由 ——
①原生 `<select>` 的寬度由最長的 option 撐出來（365px 那次）
②option 文字同時是「選中後顯示在收合狀態」的文字，補述會被截成半句。
**選項名稱講不清楚時，正解是把名稱改對。**

**驗證方式**（`npm run build`；`?v=` → `20260827005`；**沒有動資料庫、沒有動 C#**）

- 下拉四個選項：`📝 有時程異動 (1)` / `⏰ 有延期完成 (1)` / `⏰ 延期完成 2 次以上 (0)` /
  `🔄 有規格回退 (1)` —— **沒有任何一個帶尾巴**。
- 選中後收合狀態顯示 `警示：⏰ 有延期完成 (1)`，不再是被截斷的半句。
- 晶片 `警示｜有延期完成｜✕`、圖例 `⏰ 延期完成次數（2 次以上轉紅）`，用語一致。
- 篩選行為沒變：`?alert=delay` 仍然篩出 **NID 6**（列上 `⏰1`）。
- 版面沒退回去（1440）：工具列高 **60px**、單行、警示下拉 **140px**。console 無錯誤。
- ⚠️ **量測陷阱**：`resize_window` 的 `preset:"desktop"` 之後 `innerWidth` 量到 **0**，
  工具列因此「每個控制項各佔一行、高 466px」—— 那是量測環境的假象，不是版面壞掉。
  量之前先明確設一次寬度。

### ✅ 第 36 批（修工具列破版）—— **完成，無 SQL 腳本**（2026-08-27）

使用者附截圖：**「此版面太不符合 UIUX，請修正」**。
⚠️ **這是第 34 批造成的迴歸，不是原本就長那樣** —— 我自己弄壞的。

**根因：原生 `<select>` 的寬度由「最長的那個 option」撐出來，不是由選中的值。**
第 34 批在「警示」的選項後面補了一句 22 字的說明，那一顆立刻從 ~140px 變成 **365px**。
實測（1280 視窗 / 工具列 1217px）：

| | 搜尋 | 漏斗 | EMS | MSD | 到期 | 進度 | 警示 | 動作 |
|---|---|---|---|---|---|---|---|---|
| 壞掉時 | 189 | 34 | 104 | 105 | 185 | 138 | **365** | 168 → **擠到第二行** |
| 修好後 | 220 | 34 | 140 | 140 | 140 | 140 | **140** | 168 → 同一行 |

畫面上的症狀有兩個：①五顆下拉寬度參差不齊 ②`Excel` 與「＋新增需求」被擠到第二行，
多出一整條幾乎空白的列（工具列高度 **102px → 60px**）。

**修法**

1. **`select.ctl` 固定寬度 140px + `text-overflow: ellipsis`**（`input.css`）。
   固定之後選項文字再長也影響不了版面，下拉展開時仍然是完整文字 ——
   那才是使用者實際閱讀說明的地方。
2. **搜尋框 `max-w` 由 280 收到 220**。它是 `flex-1`，會把剩餘空間吃光；
   五顆等寬之後固定部分變寬，280 會剛好又把動作區擠下去
   （第一次只改下拉寬度時實測搜尋框自己長到 280，**仍然是兩行**）。
3. **第 34 批那句說明縮短**：`＝按下「✓ 完成」時已超過原訂結束日` → `＝逾期後才按完成`。
   ⚠️ 理由不只是寬度：**option 的文字同時是選中之後顯示在收合狀態的文字**，
   22 字的完整版選下去會把整顆下拉塞滿並被切掉。
   完整版沒有消失 —— `<select>` 的 `title` 與圖例列的 tooltip 都還寫著那一句。
   「延期 2 次以上」的 `＝同上…` 整句拿掉（標籤本身已經說完了）。

> ⚠️ **算式要記住**（1217px 工具列）：`220 + 34 + 1 + 140×5 + 168 + 8 個 gap×8 = 1187`，
> 只剩 **30px** 餘裕。**動搜尋框 max-w／下拉寬度／動作區內容任何一個都要回來重算**，
> 否則又會多出一條幾乎空白的第二列。這種破版是靜默的，只有某些視窗寬度才看得出來。

**驗證方式**（`npm run build`；`?v=` → `20260827004`；**完全沒有動資料庫、沒有動 C#**）

- **1280**：九個控制項全部落在 `top: 102`（同一行），工具列高 **60px**（壞掉時 102px）。
- **1440**（主要工作機）：同樣單行、高 60px，警示下拉 140px。
- **1120**：動作區整組換到第二行 —— 真的太窄，換行是對的，而且是**整組一起**換。
- 選項與說明都還在：`警示：⏰ 有執行延期 (1)　＝逾期後才按完成`，
  `title` 仍是「…是按下「✓ 完成」時已超過原訂結束日的結果」。console 無錯誤。

### ✅ 第 35 批（軌跡收合 + 回退後重排的分類）—— **完成，`14` 腳本已執行**（2026-08-27）

使用者附截圖問兩件事：**「我回退一次規格，軌跡這樣紀錄是否太肥？」**
與 **「回退之後的壓日期，好像沒有寫入在軌跡內」**。第二個是真的 bug。
⚠️ **這批有動 `Program.cs`**（前面第 27~34 批都沒有），跑過 `dotnet build`：0 警告 0 錯誤。

**一、軌跡太肥 —— 資料是對的，冗的是渲染**

寫入端**不要改**：回退清掉 N 個階段就得留 N 筆快照，少一筆就不知道當時清掉了什麼。
而且它已經很節制（`Program.cs` 的 `if (!AnyDate(oldD)) continue;`，本來就沒日期的階段不留紀錄）。
冗的是前端逐筆各畫一個區塊 —— 四筆的**時間／異動人／分類／說明完全一樣**，
那句 33 字的說明在畫面上重複四次，**同一次動作被畫成四件事**。

修法：`changeGroups` —— 相鄰且「型別＋時間＋人＋分類＋說明」全同的收成一張卡，
共用資訊只畫一次，內容變成「一個階段一行」。
- ⚠️ **只併相鄰的**：`/api/history` 是 `ORDER BY RequirementId, ChangedAt, Id`，
  同一次寫入本來就連續；跨越其他紀錄硬併會把時序畫顛倒。
- ⚠️ **單筆的群組版面一律維持舊版**（階段名在前、欄位在前、分類與說明在後）——
  單筆是絕大多數，沒有理由順手改掉它既有的讀法。只有多筆時說明才提到最前面
  （那句話解釋的是整組，擺在四個階段後面會像只註解最後一個）。
- 手法本身有先例：`initStamp` 早就是「四筆 init 時間來源都一樣，就抽到標題上講一次」。

**二、回退後壓的日期沒進軌跡 —— 有寫入，但被判成 `init`**

根因在 `WriteAuditAsync()`：`oldEnd == "" → "init"`。
**規格回退剛好會把 End 清成 NULL**，於是「重新排程」被當成「這個欄位第一次被填」。
三個後果：①那一列沉到面板最下面的「初始時程」區、標題還寫著「初始」
②不計入 ⚠N（`isDateChange` 只認 `日期異動`）
③同一階段出現兩筆 `init`，`initStamp` 的時間戳去重失效，那一區退化成每行各印一個時間。

修法：新增 ChangeType **`重新排程`**。
- `LastChangeTypeByPhaseAsync()` 一次查詢撈四個階段最後一筆的 `ChangeType`
  （`ROW_NUMBER` 取每組最新）。⚠️ **不可在迴圈裡一個階段查一次** —— 匯入是逐列呼叫
  `WriteAuditAsync` 的，那會變成每列多送四趟。⚠️ 一定要吃同一個 `tx`。
- ⚠️ 新增（`oldReq == null`）**跳過查詢**：那時還沒有任何稽核列，全部都是真的 `init`。
- 判定只看「這個階段最後一筆是不是 `規格回退`」—— 會把 End 清空的路徑只有回退
  （手動清掉既有日期時 `oldEnd` 不是空的，會落在 `日期異動`）。
- **不計入 ⚠N**（沒有人改動任何既有日期，計進去會讓同一件事被數兩次）、
  **不強制理由**（正上方那筆回退自己就帶著回退說明，再逼一次是同一件事問兩遍）；
  但使用者真的填了就照收（`keepMeta` 含 `重新排程`）。
- ⚠️ **刻意不併進 `日期異動`**：那會讓 ⚠N 灌進一種性質不同的事件，
  而且畫成「未填 → 2026-09-15」讀起來仍然不像重新排程。

**驗證方式**（`dotnet build` 0/0；`npm run build`；`?v=` → `20260827003`）

- 造暫時資料 `ZZTEST35`（stage 3、①②③ 有日期）→ `POST /rollback` 退到 1 →
  `PUT` 補上 ① 的日期。DB 實測第 249 列是 **`重新排程`**（不是 `init`）。
- 畫面：回退收成**一張卡**（`規格回退 / 22:43 / · 影響 3 個階段 / 規格變更 / 說明×1`
  + 三行階段），重新排程獨立一張卡並**留在「時程變更軌跡」**（不再沉到初始時程），
  「初始時程」回到共用一個時間戳的收合狀態，NID 欄只有 `🔄1`、**沒有 ⚠**。
- **單筆沒被改壞**（NID 6）：`③ MSD開發中 / 日期異動 / 22:13 · yu-tinglin` →
  `結束 2026-08-31 → 2026-08-12 提前 19 天` → `其他` → `說明：test`，順序與改之前完全一致；
  同一列的 `延期完成` 仍然畫成「原訂 → 實際 延後 15 天」。
- console 無錯誤。**暫時資料已刪除**（62 筆 active、`ZZTEST3%` 殘留 0）。

**✅ `14_fix_reschedule_changetype.sql` 已執行**（使用者同意後）。修正 **1 列**
（`Id 242` / NID 4 / spec / 2026-08-27 22:21，`init` → `重新排程`），
殘留 0、重跑冪等已複驗。畫面複驗：那筆已從「初始時程」移進「時程變更軌跡」，
「初始時程」收合回共用一個 `2026-08-23 00:38 · Excel 匯入` 標題、① 不再出現兩次。

> ⚠️ **腳本第一次執行時 `FixedRows` 印出 0，但那一列其實已經改成功了。**
> 原因：`SELECT @@ROWCOUNT` 被放在 `PRINT` 後面，而 **`PRINT` 會重設 `@@ROWCOUNT`**。
> 與 `DB_table.md` 記的 `07` 那個「回填 0 筆但腳本跑完了」是同一類坑，方向相反
> （做了事卻回報 0）—— 重跑的人看到 0 會分不出「早就修好了」和「根本沒作用」。
> 已改成緊接著 `UPDATE` 就 `SET @Fixed = @@ROWCOUNT`。
> 修完驗過兩件事：重跑報 0（真的沒事可做）、以及在一個 `ROLLBACK` 的交易裡把 242
> 暫時設回 `init` 再跑同一段邏輯 → 正確印出 **1**，回捲後 242 仍是 `重新排程`。
> **日後所有累加腳本要回報影響列數，一律緊接著 DML 收進變數。**

### ✅ 第 34 批（「延期」在選單裡補一句說明）—— **完成，無 SQL 腳本**（2026-08-27）

使用者問：**「我目前的網頁沒有延期的功能，怎麼會有延期的選項？」** 他是對的 ——
**畫面上沒有任何一顆按鈕叫「延期」**。它是「✓ 完成」按下去那一刻的判定結果：
今天 > 原訂 End → `延期完成`（原訂 End 不動、只寫 `ActualEnd`、`DelayCount +1`）；
今天 ≤ 原訂 End → `提早／準時完成`（End 更新成今天、`EarlyCount +1`）。
⚠️ **沒有動任何 C# 檔。**

**在此之前的排查（結論：功能一直是好的，只是沒有資料 + 詞彙對不上動作）**

- `matchAlertFilter()`（[app.jsx:2584](ClientApp/app.jsx:2584)）有接進 `filteredData`，
  `?alert=delay` 實測會篩成 0 筆並顯示空狀態 —— **它有反應，只是反應是一張空表**。
- 後端寫入路徑也都在：`/done` [Program.cs:1590](Program.cs:1590)、`/rollback` 1743。
- 用暫時值（NID 1 設 `DelayCount=2, RollbackCount=1`）做過正向測試，三種模式都精準命中，
  下拉括號同步變 (1)(1)(1)。**測完已還原成 0。**
- 之所以全是 0：資料是 **2026-08-23 00:38 匯入**進來的（62 筆 `CreatedAt` 全是那一刻），
  匯入 `TRUNCATE` 重灌並重寫 `init`，在那之前的軌跡與計數全部歸零。

**修法**：`FilterSelect` 加一個 `hint`（選項後面補說明）與元件層的 `hint`（掛在 `<select>` 的 title）。

- ⚠️ **說明一定要寫進 `<option>` 的文字裡，不可以用 `<option title>`** ——
  原生 select 的下拉清單由瀏覽器／作業系統繪製，`title` 在那裡**不保證會顯示**，
  掛上去很可能靜靜地沒有作用。
- 只補「⏰ 有執行延期」與「⏰ 延期 2 次以上」兩個。**另外兩個刻意不補**：
  「🔄 有規格回退」與「📝 有時程異動」的名字本身就對得上畫面上真的存在的動作
  （編輯視窗的 `🔄 規格回退` 鈕／改日期），不會讓人去找一個不存在的功能。
- 圖例列的「⏰ 執行延期次數」補同一句 tooltip（`cursor-help`）—— 那是這個詞出現的另一個地方。

**驗證方式**（`npm run build`；`?v=` → `20260827002`；**完全沒有動資料庫**）

- 選項文字實測：`警示：⏰ 有執行延期 (1)　＝按下「✓ 完成」時已超過原訂結束日`、
  `警示：⏰ 延期 2 次以上 (0)　＝同上，且發生 2 次以上`；另外兩個沒有尾巴。
- `<select>` 的 title 是「依 警示 篩選 \n「執行延期」不是獨立功能，是按下「✓ 完成」時已超過原訂結束日的結果」。
- 圖例那顆 `cursor: help` + tooltip 正確。console 無錯誤。
- ⚠️ **使用者當天自己把三種紀錄都做出來了**（`ChangedBy = yu-tinglin`）：
  22:06 NID 20 ③ `提早完成`（08-27 比原訂 08-28 早一天 → `EarlyCount` 0→1、
  `MsdEnd` 更新成 08-27、StatusID 推到 4）、22:12 NID 4 `規格回退` ×4 phase
  （`RollbackCount` 0→1、四個階段日期全清、StatusID 退回 1）、22:13 NID 6 `日期異動`，
  之後又做出一筆 `延期完成`（NID 6，`⏰1`）。`?alert=delay` 現在實測**篩出 NID 6**。
- 🔁 **順帶驗證了第 33 批**：NID 20（③ 完成後 StatusID=4、④ 沒壓）與 NID 4（回退後日期全空、
  StatusID=1）都自動變成「⚠ 未壓日期」並排到最前面 —— 那正是第 33 批預測的兩條產生路徑。

### ✅ 第 33 批（逾期未壓：已到階段卻沒壓日期）—— **完成，無 SQL 腳本**（2026-08-27）

使用者**附截圖**要求：「已到階段但卻又未壓日期的，這個也要 highlight、
幫我併到逾期優先的最上面（算是逾期未壓）」。截圖是一筆 `StatusID = 4 EMS驗收`、
①②③ 都壓好了、**④ 那格是一個灰色的「-」**的需求。
⚠️ **完全沒有動任何 C# 檔**（`Program.cs` 一行都沒碰），所以沒跑 `dotnet build`。

**為什麼在此之前它是隱形的**：`resolveDuePhase()` 只看**有日期**的階段
（`DUE_PHASES.filter(p => isDateVal(...) && !isPhasePassed(...))`）。那一列
①②③ 全部 `isPhasePassed`、④ 沒有日期被 filter 掉 → `open` 是空的 → 回 `null`：

| 位置 | 在此之前的畫面 |
|---|---|
| 資料列 ④ 那格 | 灰色的「-」，與「這階段還很遠、當然還沒排」長得**一模一樣** |
| 左側風險色條 | 透明（`rowAlert` 是 null） |
| 需關注 / 逾期篩選 | **完全找不到它**（`dueInfo` 查無此列） |
| 逾期優先排序 | 舊碼 `av == null → return 1`，**把它排到整張表的最後面** |
| 精簡模式那一欄 | 灰字「未排定」 |

而這個狀態**按一下「③ 完成」就會產生**（`/done` 把 `StageCode` 推到 4，`UatEnd` 仍是空的），
不是罕見的髒資料。

**做法**：新增 `unsetDuePhase()`（`level='unset'`），並把入口收斂成 `resolveFocusPhase()`。

- **順序是刻意的：先問「當前這一階段壓日期了沒」，再退回 `resolveDuePhase()`。**
  反過來會漏掉「③ 沒壓、但 ④ 已經先填了預設驗收日」——
  那時 `resolveDuePhase` 會挑到 ④（日期在很遠的未來、連 7 日窗都進不去），
  畫面指著一個還沒輪到的階段，真正卡住的 ③ 一個字都沒提。**已實測**（見下）。
- **不受 `windowDays` 限制**：它不是「N 日內到期」，是「連 N 都還沒有」。
  7 日窗的 `dueAlerts` 與超大窗的 `dueInfo` 都一定收得到，否則 KPI 與篩選又會各算各的。
- ⚠️ **不可以把它塞成一個很小的 `diffDays`（例如 -9999）去混進同一條數線** ——
  那個假天數會流進畫面（`AlertItem` 會印「逾期 9999 天」）與 `matchDueFilter` 的
  `diffDays < 0`（它就被算成「已逾期」，而它並沒有任何逾期的日期可查）。
- ⚠️ `matchDueFilter` **全部改成比 `e.level`**：`unset` 的 `diffDays` 是 `null`，
  而 **`null <= 7` 在 JS 裡是 `true`**、`null < 0` 是 `false` —— 舊寫法會讓它剛好
  混進「需關注」卻進不了任何一個細項，而且完全看不出是靠強制轉型碰對的。
- ⚠️ `StageCode` 空白或超出 1~5 的**一律不推斷**。空白代表「不知道走到哪」，
  硬猜一個階段說它未壓只會冤枉一批舊資料；壞值那一格本來就已經有紅色的 ⚠ 在請人修。
- ⚠️ 已被下一階段接手的（`isPhasePassed`，含有 `*ActualEnd` 的）不算未壓，是**不用壓**。
  這一道正是第 23 批那個「StatusID=2、① 逾期、② 沒排日期卻算一件需關注」的反例
  不會再回來的原因 —— 而且**每一件被算進去的，資料列上都看得見紅色的「⚠ 未壓日期」**。

**畫面改了哪些**（一般模式 + 精簡模式 + 統計報表三處都要對得起來）

| 位置 | 改成 |
|---|---|
| 資料列該階段那格 | 紅底實線框的「⚠ 未壓日期」徽章（`UnsetDateBadge`），tooltip 講原因 |
| 左側風險色條 | 紅色，且 `pickRowAlert` 把 `unset` 排在 `overdue` **前面** |
| 精簡模式「目前階段時程」 | 同一顆徽章 + 階段名（在此之前是灰字「未排定」，與「四階段都沒排」分不出來） |
| 逾期優先排序 | `dueRank`：0=未壓 → 1=有到期日（按天數） → 2=沒有到期資訊 |
| 工具列「需關注」鈕 | 多一段「· 未壓 N」（0 時不出現），排在「· 逾期 N」前面 |
| 逾期下拉 | 多一個「已到階段未壓日期 (N)」，網址 `?due=unset` |
| 統計報表 KPI「需關注」 | 副標變「未壓 N · 逾期 N · 7 日內 N」 |
| 風險預警卡 | 排最上面，右欄印「未壓日期 / 尚未壓定」（`dueLabel(null)`） |
| 圖例列 | 多一句「⚠ 未壓日期＝已到該階段卻還沒壓日期」 |

**驗證方式**（`npm run build`；`?v=` → `20260827001`；**資料庫已完整還原**）

- ⚠️ **全表 62 筆一筆都不符合**（每一筆當前階段都有日期），所以造了暫時資料 `ZZTEST33`
  重現截圖那個狀態：`StageCode=4`、①②③ 有日期、④ 空。
- **主情境**：`?dp=1` → **ZZTEST33 排第 1**，後面才是 12 / 61 / 62（逾期 -7/-6/-6）
  再接 7 筆琥珀。④ 那格是「⚠ 未壓日期」（`color`/`borderColor` 都是 `rgb(248,113,113)`），
  左側色條紅色。工具列變「需關注 11 · 未壓 1 · 逾期 3」。
- **順序的實測**：用 SQL 把同一筆改成 `StageCode=3`、`MsdEnd=NULL`、`UatEnd=2026-12-31`
  （① 未壓 + ④ 已預填）→ 徽章正確落在 **③ MSD開發中**、④ 照常顯示 `2026-12-31`，
  仍排第 1。⚠️ 這個組合**用 API 建不出來**（gating 回 400「請先完成 3_MSD開發中 的日期」），
  只有匯入或回退進得來 —— 但那正是它值得測的理由。
- **四個篩選互斥且加得起來**：`?due=overdue` → 12,61,62（**未壓那筆不在裡面**，
  它沒有任何逾期的日期）；`?due=soon` → 7 筆；`?due=unset` → 只有 ZZTEST33；
  `?due=attention` → 11 筆 = 3 + 7 + 1。網址參數與晶片（「到期：已到階段未壓日期」）都對。
- **精簡模式**：9 欄，「目前階段時程」那格是「⚠ 未壓日期 / ④ EMS驗收」。
- **統計報表**：KPI「需關注 11 / 未壓 1 · 逾期 3 · 7 日內 7」；
  風險預警卡第一筆是「ZZTEST33 · ④ EMS驗收 · 未壓日期 / 尚未壓定」。
- **沒有誤傷**：全表只有 1 顆徽章；**NID 49**（`StageCode=5` + `MsdEnd` 空，那筆階段跳空的
  活教材）**沒有被標**（結案先擋）；45 筆 Done 全部無感。
- **還原後複驗**（DB 刪掉 id 67 與它的 3 筆稽核列）：62 筆、**0 顆徽章**、
  「需關注 10 · 逾期 3」（`· 未壓` 那段自己消失）、色條 3 紅 7 琥珀 52 透明、console 無錯誤。
  `dbo.Controltable` 62 筆 active，`Controltable_History` 224 筆
  （⚠️ 另有 1 筆 `IsDeleted=1` 的 **ZZTEST26**，那是第 26 批留下的，**不是這批的**）。

### ✅ 第 32 批（投影模式改成有前置條件）—— **完成，無 SQL 腳本**（2026-08-24）

使用者要求：**「只有精簡模式下才能用投影模式，切到其他頁面就回復成正常版面」**。
⚠️ **一樣沒有動任何 C# 檔。**

**為什麼這是對的**：第 30、31 批各修了一次「版面跑掉」，但兩次的根源都是同一件事 ——
**「投影 + 16 欄」這個組合存在**。只要它組得出來，可用寬度（視窗 ÷ 倍率）
就一定小於 16 欄表格的 1237px，整頁橫捲、頁首與工具列跟著滑走。
在此之前是「按下投影就順手幫你打開精簡模式」（借用），借用有漏洞（載入路徑、
使用者手動關掉精簡）；改成**硬性前置條件**之後那個組合在畫面上根本組不出來，
不必再靠事後偵測補救。

| 情境 | 行為 |
|---|---|
| 非精簡模式 | `📽 投影` **disabled**，tooltip 說「請先按下精簡模式」 |
| 在統計報表頁 | `📽 投影` **disabled**（圖表放大會被擠扁） |
| 投影模式中 | `精簡模式` 開關 **disabled**；`toggleCompact()` 另有 `if (present) return` 當最後一道 |
| 投影中切到統計報表 | **自動退出投影**；切回需求列表**不會**自動再開 |
| 載入時 `present && !compact` | **直接退出投影**（第 30 批是「補開精簡模式」，這批改成退出） |

- `beforePresent` 從 `{ dark, compact, duePriority }` 縮成 **`{ dark }`** ——
  精簡模式已經是前置條件、逾期優先由 `toggleCompact()` 自己處理，不再需要借用還原。
- 「⚠ 右邊被切掉」那顆順手補了 `'none'` 分支：投影中已經是精簡模式、倍率也降到底時
  沒有東西可按，改成 disabled 的純提示「請把瀏覽器視窗拉寬」
  （在此之前會顯示「點一下切換精簡模式」但按下去不會有任何事）。
- `ToggleChip` 新增 `disabled` prop（目前只有精簡模式在用）。

**驗證方式**（`npm run build`；`?v=` → `20260824014`；**完全沒有動資料庫**）

- 非精簡下 `投影` **disabled**、點了沒反應；開精簡後解鎖；進投影 → 9 欄 overflow **0**，
  `精簡模式` 變 disabled、點了關不掉（tooltip 寫出原因）。
- **切到統計報表** → 投影自動關閉（`zoom` 回 1、`max-w` 回 1440、`ct.presentMode` → `"0"`）；
  切回需求列表 **不會**自動再開，精簡模式仍在。
- 統計報表頁的 `投影` 是 disabled，點下去不會進投影（`zoom` 維持 1）。
- **深色仍然是借用**：深色 → 進投影變淺色 → 離開**還原成深色**。
- **舊的壞組合**（localStorage `present=1` + `compact=0`）重新整理 →
  **退出投影、16 欄、overflow 0、標題可見**，`ct.presentMode` 被收成 `"0"`。

### ✅ 第 31 批（修「UI 在非精簡模式會跑掉」）—— **完成，無 SQL 腳本**（2026-08-24）

使用者又附截圖回報：**非投影、字級 130%、16 欄**，頁首與 StatusID 那排都被切在畫面左邊。
⚠️ **一樣沒有動任何 C# 檔。**

**根因：與第 30 批是同一個。** 可用寬度 = 視窗寬 ÷ zoom 倍率，而 16 欄的表格
**壓縮到極限是 ~1237px**；低於它就整頁橫捲，頁首與工具列跟著滑走。
第 30 批只修了「投影倍率」那條路，**沒回頭檢查第 29 批自己加的字級**也會吃掉同一個寬度 ——
這次是我造成的迴歸。實測（1440 螢幕 / 16 欄）：

| 情境 | 可用寬度 | 表格寬 | overflow |
|---|---|---|---|
| 100% | 1425 | 1375 | **0** |
| 字級 115% | 1239 | 1240（已壓到極限） | 30px |
| 字級 130% | 1096 | 1237（壓不下去） | **216px** |
| 投影 150% | 950 | 1238 | **470px** |
| 9 欄（精簡）| — | 901 | **0** |

**修法**：把第 30 批那顆「⚠ 右邊被切掉」**從投影模式推廣到整個需求列表**
（`presentClip` → `clipPx`，`activeView === 'table'` 就量），
並依情境給**最有效**的一鍵修正：

| 情境 | 主要動作 |
|---|---|
| 投影中（倍率還能降） | 降一級投影倍率 |
| 字級 > 100% | 降一級字級（`stepUiScaleDown`，**不繞回** —— `cycleUiScale` 在 115% 會跳到 130%，更糟） |
| 其餘（含投影已在最低倍率） | 切換精簡模式 |

- 次要選項寫在 tooltip，不長第二顆鈕。字級鈕的 tooltip 也補上這件事。
- ⚠️ **刻意不自動幫使用者改設定**：放大字級是他自己按的，靜靜把欄位收起來更難理解。
  （投影模式載入時「借用精簡模式」是唯一例外，那是第 30 批定的。）

**驗證方式**（`npm run build`；`?v=` → `20260824012`；**完全沒有動資料庫**）

- **字級分支**（1440 / 16 欄）：100% overflow 0 無警告 → 115% overflow **30**、警告出現
  → 130% overflow **216**，tooltip 寫「原因是字級放大後可用寬度變成『視窗寬 ÷ 130%』，
  16 欄的表格放不下」→ 點兩下 130→115→100，overflow 216→30→**0**，警告消失。
- **精簡分支**（1200 / 100% / 16 欄）：overflow **84** → tooltip 改成「點一下切換精簡模式」
  → 點下去 **9 欄、overflow 0**、警告消失。
- **投影分支**（第 30 批的行為沒被改壞）：投影開啟 → 9 欄 overflow 0 無警告、字級鈕隱藏；
  手動關掉精簡 → 16 欄 overflow **470**、tooltip 改成「降一級投影倍率」→ 點下去 1.5→1.4、
  overflow 470→**342**。
- **統計報表不出現這顆**（只在 `activeView === 'table'` 量）。
- 收尾狀態：非投影、16 欄、overflow **0**、無警告。

### ✅ 第 30 批（修「投影的情況下版面會跑掉」）—— **完成，無 SQL 腳本**（2026-08-24）

使用者**附了一張截圖**回報：投影模式下頁首標題只剩「管控表」、搜尋框被切掉，
但表格的 No / NID 還在原地。⚠️ **一樣沒有動任何 C# 檔。**

**根因（已完整重現）**：「借用精簡模式」只寫在 `togglePresent` 裡，
而 `present` 是從 `localStorage`（`ct.presentMode`）復原的 ——
**投影模式開著時按 F5，借用不會跑到**，畫面就落在「投影 1.5 倍 + 16 欄」
這個投影模式從來沒有被設計過的狀態。

- 實測（1440 螢幕 × 150%）：可用寬度 = 視窗 ÷ 倍率 = **950px**，
  而 16 欄表格的最小寬度是 **1238px** → 整頁橫捲 **470px**。
- 頁首與工具列是**整頁**的一部分，橫捲時一起滑出畫面左邊；
  只有表格的左側凍結欄（第 27 批）留在原地 —— 所以看起來特別像「跑掉」。
  切成 9 欄（901px）overflow **立刻是 0**。
- ⚠️ 這不是第 27 批的 `max-w-[1600px]` 造成的：那個上限在這個寬度根本沒生效
  （可用 950px 遠小於 1600），一般模式 1440 也量到 overflow **0**。
  第 27 批的凍結欄只是讓「本來就存在的橫捲」變得更明顯。

**修法三件**

1. **載入時也借用一次**（`presentBootRef` 的 mount-only effect）：`present` 為真就套
   淺色 + 精簡 + 逾期優先，並把「進來之前」記進 `beforePresent` ——
   在此之前重新整理後那個 ref 是空的，離開投影時什麼都不還原。
2. **投影模式下不套 `max-w` 上限**（`pageWidth` 改回 `max-w-none`）：
   可用寬度是「視窗 ÷ 倍率」，1600 只有在寬螢幕（2560 ÷ 1.25 = 2048）才生效，
   而那正是最需要把表格攤開的場合，卻反而被切成 1600 並置中留白。
3. **真的塞不下時主動講**：頁首出現「⚠ 右邊被切掉」（帶實際 px 數），點一下降一級倍率；
   已經最低時改口「請改用精簡模式、或把瀏覽器視窗拉寬」。
   **台上的人看自己的螢幕，不會發現布幕右邊少了幾欄** —— 這是唯一會讓人發現的方式。

**驗證方式**（`npm run build`；`?v=` → `20260824011`；**完全沒有動資料庫**）

- **先完整重現**：把 localStorage 設成 `presentMode=1` / `compactMode=0` / `presentZoom=1.5`
  再重載 → 16 欄、overflow **470px**、頁首標題跑到 `left: -371`（截圖那個狀態）。
- **修完重載同一組值** → **9 欄、overflow 0**、標題 `left: 99`（看得見）、深色自動關掉。
- **離開投影** → 還原成進來之前的偏好（16 欄、`ct.compactMode` 回到 `"0"`）；再進去 → 9 欄、overflow 0。
- **警告**（1200 螢幕，使用者自己在投影下關掉精簡模式）：16 欄 → overflow **710px** →
  「⚠ 右邊被切掉」出現，tooltip 寫「畫面右邊有 710px 在視窗外…點一下降一級倍率」；
  連點兩下 1.5 → 1.4 → 1.25，overflow 710 → 582 → **395**，最低倍率時 tooltip 改口；
  精簡模式打回去 → overflow 0、警告消失。
- **一般模式沒被影響**（1440）：16 欄、表格 1375px、overflow **0**、凍結欄 `left: 48px` 正常。
- ⚠️ **試過但行不通的做法**（別再走一次）：外層 wrapper 套 `width: max-content` + 頁首內層
  `sticky left-0`，想讓頁首在橫捲時不滑走 —— **overflow 反而從 470 變 840**
  （max-content 把 wrapper 撐到表格的 max-content 寬度），標題照樣滑走。
  「整頁捲動」是 2026-08-19 拍板的（表格不可再包 overflow，否則兩層 sticky 表頭失效），
  在那個前提下頁首本來就會跟著橫捲 —— **正解是不要讓它橫捲**，不是想辦法釘住頁首。
- ⚠️ **又踩到一次背景分頁的量測陷阱**：警告原本包在 `requestAnimationFrame` 裡，
  分頁在背景時 rAF 不會被呼叫 → overflow 明明 710px 警告卻**靜靜地不出現**。
  改成同步量（`useEffect` 跑的時候 DOM 已經 commit，讀 `scrollWidth` 本來就會強制排版）。

### ✅ 第 29 批（UI/UX 的 D + E + F + G）—— **完成，無 SQL 腳本**（2026-08-24）

使用者說「都修正」。⚠️ **一樣沒有動任何 C# 檔**（`Program.cs` 完全沒碰）。
規格細節寫在 **`FIELD_SPEC.md` 的「可及性、字級與窄螢幕」**那一節，這裡只記結論與踩到的坑。

**D. 可及性**（改之前全檔 `aria-label` / `role` / `tabIndex` 各 **0** 個）

1. **展開明細改成真的 `<button>`**（`No` 欄的三角形），`aria-expanded` + 帶 NID 的 `aria-label`。
   在此之前只有 `<tr onClick>`，**鍵盤展不開任何一列**。
   ⚠️ 刻意**不**把 `role="button"` + `tabIndex` 掛到 `<tr>` 上 —— 那會讓 62 列的表格在
   無障礙樹上從「資料列」變成「按鈕」，整個失去列／欄結構。
   ⚠️ button 內要 `stopPropagation`，否則點一下會展開後立刻被 `<tr>` 的 onClick 收合。
2. **可排序表頭**：15 處各自寫的 `onClick={()=>requestSort('x')}` 收斂成 `sortProps()`
   （`tabIndex` + Enter/Space + `aria-sort`）。同樣**不加** `role="button"`（`<th>` 是 columnheader）。
   ⚠️ Space 一定要 `preventDefault`，否則按下去會順便把整頁往下捲一頁。
3. **六個 Modal 共用一份焦點管理**（`data-ct-modal` + `role` + `aria-modal` + Tab 環繞 + 焦點歸位）。
   ⚠️ **踩到的坑**：原本在 effect 裡讀 `document.activeElement` 當「開窗前的焦點」——
   React 的 `autoFocus` 在 commit 階段就套用了，**比 `useEffect` 早**，
   讀到的是視窗裡的 NID 輸入框，關窗時它已被卸載、`document.contains()` false，
   **還原焦點整條沒有作用而且失敗得很安靜**。改成用 `focusin` 一直記錄
   「最後一個不在視窗裡的焦點」（`lastOuterFocusRef`）。
   ⚠️ 也不要去搶已經 `autoFocus` 的焦點；沒人接手時聚焦視窗容器本身（`tabIndex={-1}`）——
   搶了就會踩到 FIELD_SPEC 那條「編輯時不可聚焦 NID」。
4. **圖示鈕補 `aria-label`**，裝飾性 svg 補 `aria-hidden`。
   ⚠️ 每列的編輯／刪除**要帶 NID**：62 列念 62 次「刪除」根本分不出停在哪一筆。

**E. 投影與字級**

5. **Toast**：`role="status"` + `aria-live`（錯誤 assertive）、加 ✕ 可手動關、
   **停留時間改成看字數**（約每字 90ms，夾 3~12 秒；錯誤下限 5 秒）、
   投影模式跟著放大（`present-zoom` 以前只掛 `<header>`／`<main>`，toast 在更外層，台下看不見）。
6. **字級**：頁首 `Ａ 100%` 循環 100→115→130，記在 `ct.uiScale`，只放大 `<main>`。
   ⚠️ 走 CSS `zoom`（`.ui-zoom`）**不去動那 121 個 `text-[10px]/[11px]`**：
   一個個調會動到欄寬、換行與量測出來的表頭吸附位置，而且只放大文字會讓 10px 的字
   配上沒變大的 8px 三角形更難看。⚠️ 與 `present-zoom` **不可同時掛在同一個元素上**
   （zoom 相乘，1.3×1.5=1.95）；投影模式開著時這顆按鈕整個讓位。

**F. 搜尋防抖**

7. `searchTerm`（輸入框，即時）／`searchQuery`（過濾與網址，慢 200ms）。
   在此之前打一個字就重跑 filter + 五個 `useMemo`。⚠️ 網址也要吃防抖後的值，
   不然 `replaceState` 會被打字節奏推著一個字元覆寫一次。

**G. 窄螢幕（唯一的 RWD）**

8. ≤1024px 自動套精簡模式（9 欄），開關標「· 窄螢幕」+ tooltip 說明。
   ⚠️ 用衍生值 **`compactPref || narrow`**，**不可以** `setCompactPref(true)` ——
   那會把使用者偏好蓋掉並寫進 `localStorage`，視窗拉寬之後回不去。
   投影模式的存／還原也跟著改存 `compactPref`（存 `compact` 的話，在窄螢幕進出投影一次，
   「不要精簡」的偏好就被還原成 true）。
   ⚠️ **斷點是 1024 不是 1440**：1366/1440 的筆電是主要工作機，那裡要看的是完整 16 欄
   （第 27 批的左側凍結就是為它做的），在那個寬度自作主張收成 9 欄會把欄位藏掉。
   ⚠️ `matchMedia` 的 `change` **一定要配 `resize` 備援**（見下方驗證）。

**驗證方式**（`npm run build`；`?v=` → `20260824009`；**完全沒有動資料庫**）

- a11y 計數（1280px 一般模式）：`aria-label` **189**、`aria-expanded` **63**、
  `aria-sort` **13**、`tabindex` **13**、資料列展開鈕 **62**（改之前三者都是 0）。
- 展開鈕：聚焦 → 觸發 → `aria-expanded` `false→true→false`、展開列數 `0→1→0`、
  **焦點全程留在那顆鈕上**（沒有跑掉）。
- Modal：開窗後 `role=dialog` / `aria-modal=true` / `aria-label=新增資料列`，
  焦點在視窗內（新增時是 NID，**編輯時不是** —— 落在容器 DIV 上，符合 FIELD_SPEC）；
  Tab 從最後一個環繞回第一個、Shift+Tab 反向、焦點被移到視窗外時**會被抓回來**；
  Esc 關閉後**焦點回到開窗的那顆鈕**（`activeIsAddBtn: true`）。
- Toast：`role=status`、`aria-live=polite`、✕ 可立即關閉、非投影時 `zoom:1`；
  投影模式的 `present-zoom` 對 `position:fixed` 的元素實測確實生效（zoom 1.5）。
- 字級：100→115→130→100 循環，`<main>` 的 computed `zoom` 跟著變，
  且每一級的 `--frz-2` 都等於當下量到的 No 欄寬度（48／47／47）。投影模式下按鈕不出現。
- 窄螢幕：820px → **9 欄**且開關顯示「✓ 精簡模式 · 窄螢幕」；拉回 1280 → **16 欄**、
  標記消失；全程 `localStorage.ct.compactMode` 停在 **`"0"`**（偏好沒被污染）。
  進出投影模式後也還是 16 欄 / `"0"`。
- 防抖：連打「侑」「侑憲」期間輸入框即時更新但**列數停在 62**，200ms 後一次跳到 20，
  網址只出現最終的 `?q=侑憲`（沒有中途的 `?q=侑`）。
- ⚠️ **這個環境的兩個量測陷阱**（下次別再追）：
  1. `resize_window` 改了視窗寬度但**不會派發 `resize` 事件**，`matchMedia` 的 `change`
     也不送 —— 手動 `dispatchEvent(new Event('resize'))` 之後畫面立刻正確切換。
     真實瀏覽器兩個都會送；程式碼本身沒問題（但也因此才補上 resize 備援）。
  2. `document.hasFocus()` 是 **false**（分頁在背景），所以 `.focus()` 有時是 no-op、
     而且**不會派發 `focusin`**。驗焦點歸位要自己補一個合成的 `focusin` 事件餵給 listener。
- 截圖依舊拿不到（第九次，**別再試**）。

### ✅ 第 28 批（UI/UX 的 B）—— **完成，無 SQL 腳本**（2026-08-24）

使用者接著說「B 繼續」。⚠️ **一樣沒有動任何 C# 檔**（`Program.cs` 完全沒碰），所以沒跑
`dotnet build`。規格細節（參數表、白名單、列印行為）寫在 **`FIELD_SPEC.md` 的
「生效中的條件與可分享網址」**那一節，這裡只記結論與踩到的東西。

1. **條件晶片列**（`activeChips`，表格正上方一張卡）。八種來源全部列出來，每顆自己的 `✕`
   只移除那一條。在此之前畫面上只有一顆「✕ 清除全部」—— 想改一個條件就得全部重來。
   - ⚠️ **重點是 `colFilters`**：精簡模式收起的欄位（`Status`／註冊日期／`MP Saving`／
     四個階段時程）與一般模式沒有的（目前階段時程／現況描述），**篩選值照樣在過濾**
     （`filteredData` 不分模式）。那些晶片標 ⚠ + 紅框，列尾再寫一句
     「有 N 個條件的欄位在目前模式下是收起來的，但它仍在過濾」。
     這正是「看不到條件卻筆數變少」那個坑，在此之前唯一的線索是漏斗鈕上的數字，
     **而它連是哪一欄都不會說**。
   - 判定走 `colFilterHidden()`，與篩選列實際 render 的條件共用 `COL_FILTER_META`
     那一份定義 —— 各寫一份就會再變成「同一件事兩套規則」。
   - 「到期＝需關注」那顆的 `✕` 會**一併關掉逾期優先排序**（那兩件事本來就是同一顆鈕一起打開的）。
   - ⚠️ **刻意不在晶片列再放一顆「✕ 清除全部」**：工具列那顆就在正上方一張卡的距離、
     兩者永遠同時看得見，再放一顆一模一樣的紅鈕只會讓人以為作用不同。
2. **篩選與排序寫進網址**（`replaceState` 單向：state → 網址；只寫非預設值）。
   - ⚠️ 網址**只在載入當下讀一次**（`URL_PARAMS` 是模組層的常數）。每次 render 都讀會與
     state 互相蓋，搜尋打到一半被回捲。
   - ⚠️ **不可改成 `pushState`**：搜尋框每打一個字就是一次狀態變更，按一次「上一頁」
     只會退掉一個字元。**實測連打 5 次 `history.length` 完全沒變。**
   - ⚠️ 認不得的值一律退回預設**並從網址上清掉**（`urlOne` / `urlList` 過白名單）。
   - EMS / MSD **刻意不過白名單**（選項來自非同步載入的資料）：帶了不存在的人名時清單是
     0 筆，但晶片上寫著「EMS：某某」—— 看得到原因才改得掉。
   - 帶了 `f_*` 就自動打開欄位篩選面板。

**驗證方式**（`npm run build`；`?v=` → `20260824006`；**完全沒有動資料庫**）

- 晶片＋網址雙向：搜尋「侑憲」→ 點 StatusID 3 → 填 MP Saving「3」，網址變成
  `?q=侑憲&stage=3&f_mpSaving=3`，晶片列三顆對得起來。
- **切到精簡模式**：MP Saving 的篩選輸入框消失（實測 `mpFilterInputVisible: false`），
  它的晶片轉成紅框 + ⚠、tooltip 寫出原因，列尾出現
  「⚠ 有 1 個條件的欄位在目前模式下是收起來的，但它仍在過濾」；切回一般模式警示消失。
- 單顆移除：拿掉 MP Saving 那顆 → 只有它不見（網址、晶片、**表頭那格輸入框的值**同步清掉），
  其餘條件與筆數不受影響。
- 網址還原：`?q=侑憲&stage=3,5&due=attention&prog=ongoing&alert=delay&f_mainCat=PMS&f_mpSaving=3&sort=nid:desc&dl=0`
  → **八顆晶片全部還原**、StatusID 3 與 5 兩顆藥丸亮著、搜尋框有值、欄位篩選面板自動打開；
  `?sort=nid:desc` 單獨測 → 列序 62,61,36,35…（Done 仍沉底）；
  `?dp=1` → 列序 12,62,61,20,36,31,30,10（與 `buildDueList` 的順序一致）；
  `?view=dashboard` → 開在統計報表、寬度 1440。
- **亂改網址**：`?due=BOGUS&prog=xx&alert=<script>&stage=9,abc,3&sort=evil:desc&view=hack&f_nope=1&f_nid=7`
  → 收斂成 `?stage=3&f_nid=7`（1 筆），沒有任何注入。
- 統計報表 KPI 卡 → 列表那條路沒被影響：點「進行中 17」→ 舊條件全清、
  網址變 `?prog=ongoing`、晶片只剩一顆、「顯示 17 / 62 筆」。

### ✅ 第 27 批（UI/UX 的 A + C）—— **完成，無 SQL 腳本**（2026-08-24）

使用者再問一次「有什麼建議 UIUX 優化的地方」，我把第 26 批沒挑的項目**逐條在程式碼上複驗過**
（全部仍然成立）後重新分成 A~G，他挑了 **A + C**。⚠️ **沒有動到任何 C# 檔**，
`Program.cs` 完全沒碰，所以這批沒有跑 `dotnet build`（開發伺服器還開著，跑了只會撞檔案鎖）。

**A. 左側欄位橫向凍結 + 需求列表頁放寬**

1. **No / NID 兩欄橫向凍結**（`.frz` / `.frz-1` / `.frz-2`，規則全在 `input.css`）。
   在此之前捲到右邊看 ④ 驗收的紅字時，左邊的 NID 早就捲出畫面了。
   - ⚠️ **只能凍連續的前綴欄**。一般模式欄序是 No→NID→Status→StatusID→註冊日期→
     Main Cat→Sub Cat，**Main Cat 不與 NID 相鄰** —— 想連 Main Cat 一起凍就得先改欄序，
     那會動到 `FIELD_SPEC.md` 的資料列顯示順序，**要先問過使用者**。
   - 第二欄的 `left` 用 **量測**（`noHeadRef` → `--frz-2`），不可寫死 44px。
     **實測三個值都不一樣**：資料還在載入時 37px、載入完 42px、投影模式 41px。
     ⚠️ 相依陣列一定要含 `requirementsData.length` 與 `showColFilters` ——
     **`ResizeObserver` 對 `<th>` 這種 table-cell 不回報寬度變化**，只靠 RO 會永遠停在 37。
     （差那 5px 的後果：NID 欄蓋掉 No 欄右邊 5px，吃掉分隔線、兩位數流水號被切一角。）
   - ⚠️ **凍結欄的底色必須「不透明卡片色 + 疊上列底色」**（`background-color: var(--bg-card)`
     ＋ `background-image: linear-gradient(var(--row-bg), var(--row-bg))`）。
     直接 `background: var(--row-bg)` 會壞掉：**`--bg-row-done` 本身是 `rgba(30,41,59,0.45)`**
     （深色的 `--bg-table-expanded` 是 0.5、`--bg-table-hover` 兩種佈景都是半透明），
     而 62 列裡 **45 列是 Done** —— 半透明的凍結欄等於沒凍，右邊捲過來的欄位直接透出來變兩層字。
     這個坑踩了兩次（先在 hover 上、再在 Done 底色上），兩次都是實測量到才發現。
   - 連帶：資料列底色與 hover **全部改用 CSS**（`.row-main` / `.row-exp` + `--row-bg`），
     移除 `<tr>` 上的 `onMouseEnter` / `onMouseLeave`（62 列 ×2 個 handler），
     順手修掉舊 handler 的 bug（`onMouseLeave` 寫回 render 當下閉包裡的 `rowBg`）。
   - 順手修掉一個既有的 silent bug：篩選列的底色寫的是 **`var(--bg-table)`，那個變數從來沒被定義過**
     （`input.css` 只有 `--bg-table-hover` / `--bg-table-expanded`），與註解裡那個 `--bg-main`
     是同一類坑。改成 `--bg-card`，畫面上一模一樣，但凍結欄現在要靠它遮住捲過來的內容。
2. **需求列表頁寬 `max-w-[1600px]`**（統計報表維持 1440，`pageWidth`，頁首與 `<main>` 同一個值）。
   一般模式 16 欄自然寬度約 1524px，卡在 1440 裡等於**永遠**橫捲，而 1920 螢幕兩側各留一大條白。
   ⚠️ 兩個值都必須是完整字面量，**不可拼成 `max-w-[${w}px]`**（Tailwind 掃不到、靜靜不生效）。

**C. 重新整理鈕 + 畫面抓取時間**

3. 頁首加一顆 `↻`（`handleRefresh`）。在此之前想看別人剛存的資料只能按 F5，
   而 F5 會清掉篩選／排序／展開的列；`PUT` 撞到樂觀鎖回 409 時更需要。
   - ⚠️ `fetchReqs()` 與 `fetchHistory()` **一起抓**（只抓需求的話 ⚠N 與統計報表的
     「時程異動」會停在舊數字，與刪除／匯入同一條理由）。
   - ⚠️ **不包 `runExclusive()`** —— 那是寫入用的互斥鎖，唯讀的重抓包進去會變成
     「存檔中不能重整、重整中不能存檔」。連點由 `disabled={refreshing || isLoading}` 擋。
   - 走的是第 26 批的 `refreshing` 路徑（表格淡化 + 頁首「更新中…」），**不會**出現「資料載入中…」。
4. 頁首把兩個時間**分開講**：「資料更新」＝資料的 `UpdatedAt`（資料本身多新）、
   「畫面」＝`lastFetchedAt`（你手上這份多新）。以前只有前者，**別人存了檔而你的分頁
   開著一整個下午時它一動也不動**，看起來像「沒有人改過」。`lastFetchedAt` 只在抓取**成功**
   時更新（失敗還往前帶的話會顯示一個從來沒發生過的抓取時間）；跨午夜自動補上日期。

**驗證方式**（`npm run build`；`?v=` → `20260824005`；**完全沒有動資料庫**）

- 凍結欄（1100px 視窗、捲到底 `scrollX=163`）：No 停在 `left=0`、NID 停在 `left=42`、
  第三欄跑到 `-47`（已捲出畫面）；表頭那兩格同步停在 0 / 42；凍結欄 `backgroundColor`
  是**不透明**的 `rgb(30,41,59)`。一般模式 16 欄 `--frz-2=42px`、精簡模式 9 欄 `44px`、
  投影模式（zoom 1.5）`41px`，三種都與當下量到的 No 欄寬度**完全相等**。
- 頁寬：需求列表頁首與 `<main>` 都是 `1600px`，切到統計報表都變 `1440px`，切回來又是 1600。
- 重新整理：先篩「侑憲」（62 → **20 列**）並展開一列 → 按下去 → **20 列、展開的那列還在、
  搜尋字串還在、列序不變**，「畫面」由 `10:56` 變 `10:57`，全程 `資料載入中` 一次都沒出現。
  patch `window.fetch` 讓 `/api/requirements` 慢 1 秒回，量到中間那一段：**按鈕 `disabled`
  （opacity 0.4）、圖示 `animation-name: ctSpin`、頁首「· 更新中…」、資料列沒有消失**，
  且該狀態下**再點一次不會送出第二次**。
- ⚠️ **量測陷阱（下次別再追這個）**：Browser 分頁在背景時**動畫與 transition 不會前進**，
  於是 `getComputedStyle` 對「有掛 transition 的屬性」永遠回傳**變化前**的值 ——
  資料列有 `transition-colors`、表格淡化有 `transition: opacity .15s`，
  所以量 `tr` 的 background 與 `.t-table-card` 的 opacity 都會騙人
  （inline 屬性明明是 `opacity: 0.55`，`getComputedStyle` 回 `1`）。
  **有 transition 的屬性改讀 `getAttribute('style')`**；沒有 transition 的（如 `td.frz`）才可以直接量。
- 截圖依舊拿不到（這是第八次，**別再試**）。

### ✅ 第 26 批（UI/UX 體檢的 P0 三項）—— **完成，無 SQL 腳本**（2026-08-24）

使用者問「有什麼建議 UIUX 優化的地方」，我分成 P0~P4 列出後由他挑，**他只挑了 P0 三項**。
三項都不是美化，是會做出錯誤或誤導畫面的缺陷：

1. **寫入操作沒有「送出中」狀態** → 新增 `isSubmitting` + `submittingRef` + `runExclusive()`，
   包住儲存／完成／回退／刪除／匯入五條寫入路徑，按鈕同時 `disabled`（文字換成「儲存中…」
   「回退中…」）。⚠️ **ref 是關鍵**：同一個 tick 內的兩次點擊都讀到舊的 `isSubmitting`，
   只靠 state 擋不住。**實測**：連按三下「確認新增」→ **POST 只送出 1 次**（改之前是 3 次：
   1 筆建立 + 2 個 409「NID 重複」，而使用者剛剛明明是第一次建那筆）
2. **每次儲存後整張表閃「資料載入中…」** → `fetchReqs()` 用 `loadedOnceRef` 分流：
   首次才 `setIsLoading(true)`，之後走 `refreshing`（表格 `opacity:.55` + 頁首「· 更新中…」）。
   **實測**：存檔後全程 MutationObserver 沒有看到過「資料載入中」字樣，tbody 沒有被換掉
3. **儲存驗證一次只講一個問題** → 抽出 `validateEdit()` 回傳 `{fields, groups}`；
   彈窗一次列出全部（>1 組時標題變「有 N 類問題需要修正」、內文用【】分組），
   欄位就地標紅（`errOf` / `errBorder` / `<FieldErrorHint>`，模組層元件）。
   紅字按過儲存才顯示（`showSaveErrors`）、每次 render 重算 → 改好一欄紅字就自己消失。
   **實測**：空白新增 → 5 個「⚠ 必填」+ NID 框變紅；把 NID 打成已存在的「1」→ 那一格
   立刻變成「⚠ 這個編號已被使用」（不必再按一次儲存）；再按儲存 → 「有 2 類問題需要修正」

**驗證方式**：造一筆 NID `ZZTEST26` 的暫時資料（連按三下測防連點）→ 量測 → 走 UI 刪除流程還原。
DB 已回到 62 筆，該筆是軟刪除、刪除原因寫「P0 修改驗證用的暫時資料，測試後刪除」，
`GET /api/history` 本來就排除軟刪除的需求，所以不會污染畫面上的稽核軌跡。

**使用者沒挑、但已經確認存在的問題（下次要做從這裡撿，不用重新分析）**。
2026-08-24 第 27 批做完 A 與 C 之後重新編號成 A~G；**每一條都在第 27 批當天複驗過仍然成立**：

| 組 | 問題 | 證據 |
|---|---|---|
| ~~A~~ | ~~左側 No / NID 橫向捲走~~ / ~~`max-w-[1440px]` 兩側留白~~ | **第 27 批完成** |
| ~~C~~ | ~~沒有「重新整理」鈕~~ / ~~頁首分不出「資料更新」與「畫面抓取」~~ | **第 27 批完成** |
| ~~B~~ | ~~生效中的篩選看不見細節~~ / ~~被收起欄位的 `colFilters` 仍然生效~~ / ~~篩選排序沒寫進 URL~~ | **第 28 批完成** |
| ~~D~~ | ~~可及性是零~~ / ~~鍵盤展不開資料列~~ / ~~Modal 沒有 focus trap~~ / ~~Toast 沒有 aria-live~~ / ~~圖示鈕只有 title~~ | **第 29 批完成** |
| ~~E~~ | ~~Toast 投影不放大／3 秒讀不完~~ / ~~資料列字太小~~ | **第 29 批完成** |
| ~~F~~ | ~~搜尋沒有 debounce~~（hover 已於第 27 批改 CSS） | **第 29 批完成**。分頁／虛擬捲動 **62 筆還不需要**，而且使用者 2026-08-19 要求全部攤開 |
| ~~G~~ | ~~完全沒有 RWD~~ | **第 29 批完成**（≤1024px 自動精簡；**刻意不做卡片版**） |

> ✅ **2026-08-22 起的那份 UI/UX 體檢清單（P0 → A~G）到此全部做完。**
> 下一輪要再體檢請重新分析，不要照抄這張表。
