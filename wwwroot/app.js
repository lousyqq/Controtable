...
const UnsetDateBadge=({label,onSetDate})=>{const cls="text-[10px] font-black px-1 py-0.5 rounded whitespace-nowrap";const sty={color:ALERT_STYLES.unset.color,background:ALERT_STYLES.unset.bg,border:`1px solid ${ALERT_STYLES.unset.border}`};const tip=`目前已經走到「${label}」，但這一階段還沒壓日期。\n沒有到期日就不會有逾期提醒，所以列在「逾期優先」排序的最上面`;// ⚠️ 沒給 onSetDate ＝ 精簡模式（唯讀的主管檢視）。tooltip 一定要講出
...
const isDateVal=s=>!!s&&/^\d{4}-\d{2}-\d{2}$/.test(String(s).trim());const DUE_PHASES=[{code:'1',key:'spec',label:'① EMS規格確認',color:'#f59e0b',getDate:i=>i.spec?.end,getActual:i=>i.spec?.actualEnd,owner:i=>i.emsOwner,side:'EMS'},{code:'2',key:'confirm',label:'② MSD確認中',color:'#8b5cf6',getDate:i=>i.msd?.confirm,getActual:i=>i.msd?.confirmActualEnd,owner:i=>i.msdOwner,side:'MSD'},{code:'3',key:'msd',label:'③ MSD開發中',color:'#3b82f6',getDate:i=>i.msd?.end,getActual:i=>i.msd?.actualEnd,owner:i=>i.msdOwner,side:'MSD'},{code:'4',key:'uat',label:'④ EMS驗收',color:'#ec4899',getDate:i=>i.uat?.end,getActual:i=>i.uat?.actualEnd,owner:i=>i.emsOwner,side:'EMS'}];// 最後一個已經壓了日期的階段。後面的階段既然還沒排程，現在該盯的就是這一個
...
'撤銷完成':{label:'撤銷完成',color:'var(--tone-warn)',bg:'var(--tone-warn-bg)'}};// 軌跡上的階段名稱。'stage' 不是四個階段之一，是整筆需求的狀態調整
const timelineLabelOf=phase=>PHASES[phase]?.timelineLabel||(phase==='stage'?'狀態調整':phase);// 軌跡上的異動類型樣式。⚠️ 查不到時**不可以退回 `日期異動`** —— 那會把一個
...
const phaseCircleOf=phase=>(PHASES[phase]?.timelineLabel||'').slice(0,1)||(phase==='stage'?'狀':'?');// 同一次動作寫出來的多筆稽核列收成一組（第 35 批的 changeGroups，第 72 批搬到這裡給視窗用）。
...
const PhaseChainRow=({pk,entries,item,showLabel=true})=>{if(!entries.length)return null;const ph=PHASES[pk]||{};const clr=ph.color||'var(--text-muted)';// 'stage'（手動調整／刪除）沒有日期可串，印最後一筆的說明
...
const rows=entries.filter(isMeaningfulEntry);if(!rows.length)return null;const changes=rows.filter(h=>h.changeType!=='init'&&h.changeType!=='通知寄送');const inits=rows.filter(h=>h.changeType==='init');const notifyN=rows.filter(h=>h.changeType==='通知寄送').length;const initLine=h=>initValues(h).map(([f,v])=>`${PHASE_FIELD_LABEL[f]} ${v}`).join('、');return/*#__PURE__*/React.createElement("div",{className:"mt-3 p-2 rounded border text-[10px]",style:{background:'var(--bg-detail-card)',borderColor:'var(--bg-detail-border)',color:'var(--text-tertiary)'}},/*#__PURE__*/React.createElement("div",{className:"font-bold mb-1 flex items-center gap-1.5",style:{color:'var(--text-secondary)'}},/*#__PURE__*/React.createElement("span",{title:"\u6B21\u6578\u53EA\u8A08\u300C\u65E5\u671F\u7570\u52D5\u300D\uFF1B\u63D0\u65E9\uFF0F\u5EF6\u671F\u5B8C\u6210\u8207\u898F\u683C\u56DE\u9000\u7684\u7D00\u9304\u4ECD\u5728\u4E0B\u65B9\u7684\u65E5\u671F\u93C8\u4E0A"},"\u7570\u52D5\u7D00\u9304 (",rows.filter(isDateChange).length," \u6B21)"),notifyN>0&&/*#__PURE__*/React.createElement("span",{className:"font-normal",style:{color:'var(--text-muted)'},title:"\u901A\u77E5\u4E0D\u7B97\u6642\u7A0B\u8B8A\u66F4\uFF0C\u9010\u7B46\u7684\u901A\u77E5\u7D00\u9304\u5728\u5C55\u958B\u660E\u7D30\u7684\u300C\u5DF2\u901A\u77E5 N \u6B21\u300D\u6458\u8981\u88E1"},"\xB7 \u2709 \u901A\u77E5 ",notifyN," \u6B21"),onOpenFull&&/*#__PURE__*/React.createElement("button",{type:"button",onClick:onOpenFull,className:"ml-auto ctl-sm text-[10px]",style:{height:'20px',padding:'0 6px'},title:"\u958B\u4E00\u500B\u8996\u7A97\u5217\u51FA\u9019\u500B\u968E\u6BB5\u6BCF\u4E00\u7B46\u7A3D\u6838\u7D00\u9304\uFF08\u6700\u65B0\u7684\u5728\u6700\u4E0A\u9762\uFF09"},"\u5B8C\u6574\u8ECC\u8DE1 \u2197")),changes.length>0&&/*#__PURE__*/React.createElement("div",{className:"text-[11px]"},/*#__PURE__*/React.createElement(PhaseChainRow,{pk:phaseKey,entries:changes,item:item,showLabel:false})),inits.length>0&&/*#__PURE__*/React.createElement("div",{className:"mt-1 whitespace-pre-wrap break-words",style:{color:'var(--text-muted)'},title:inits.map(h=>`${h.changedAt}${h.changedBy?` · ${h.changedBy}`:''}：${initLine(h)}`).join('\n')},"\u9996\u6B21\u586B\u5BEB ",inits[0].changedAt,inits[0].changedBy?` · ${inits[0].changedBy}`:'',inits.map(h=>` ｜ ${initLine(h)}`).join('')));};// 前置階段未完成的鎖（第 14 批）。⚠️ 與「已有值防誤改」那把鎖語意完全不同：
...
const AssigneeErrorHint=({error})=>{if(!error)return null;return/*#__PURE__*/React.createElement("div",{className:"text-[10px] mt-1 font-bold",style:{color:'var(--tone-alert)'},title:"\u8ACB\u91CD\u65B0\u6574\u7406\u9801\u9762\uFF1B\u82E5\u6301\u7E8C\u5931\u6557\uFF0C\u4EE3\u8868\u5F8C\u7AEF\u7684 /api/assignees \u6216 dbo.Assignee \u6709\u554F\u984C"},"\u26A0 ",error);};// 儲存前驗證沒過的欄位，就地標紅（2026-08-23 / 第 26 批）。
...
const FieldErrorHint=({msg})=>msg?/*#__PURE__*/React.createElement("div",{className:"text-[10px] mt-1 font-bold",style:{color:'var(--tone-alert)'}},"\u26A0 ",msg):null;// 還沒壓結束日時，完成鈕不會出現 —— 但畫面上什麼都不說的話，
...
const DoneButton=({onClick,title})=>/*#__PURE__*/React.createElement("button",{type:"button",onClick:onClick,title:title,className:"inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-bold border transition-colors",style:{color:'var(--brand)',background:'var(--brand-soft)',borderColor:'var(--brand)'}},"\u6A19\u8A18\u5B8C\u6210\u2026");// 解鎖後改了日期時要填的「異動原因分類 + 文字說明」。
// 兩者都會寫進 dbo.Controltable_History（ReasonCategory / Note）
const ReasonFields=({phaseKey,categories,setCategories,reasons,setReasons,error})=>/*#__PURE__*/React.createElement(React.Fragment,null,/*#__PURE__*/React.createElement("label",{className:"block text-xs font-bold text-red-600 dark:text-red-400 mb-1.5"},"\u26A0\uFE0F \u8ACB\u586B\u5BEB\u7570\u52D5\u539F\u56E0 (\u5FC5\u586B)"),/*#__PURE__*/React.createElement(FieldErrorHint,{msg:error}),/*#__PURE__*/React.createElement("div",{className:"flex flex-wrap gap-1.5 mb-2"},REASON_CATEGORIES.map(c=>{const on=categories[phaseKey]===c;return/*#__PURE__*/React.createElement("button",{key:c,type:"button",onClick:()=>setCategories({...categories,[phaseKey]:on?'':c}),className:"px-2.5 py-1 rounded text-[11px] font-bold transition-colors border",style:on?{background:'rgba(239,68,68,0.12)',color:'#ef4444',borderColor:'#ef4444'}:{background:'var(--bg-main)',color:'var(--text-tertiary)',borderColor:'var(--border-table)'}},c);})),/*#__PURE__*/React.createElement("input",{type:"text",className:"w-full px-3 py-1.5 rounded text-sm border outline-none focus:ring-2 ring-red-500/50",style:{background:'var(--bg-main)',borderColor:'var(--border-table)'},placeholder:"\u6587\u5B57\u8AAA\u660E\uFF1A\u70BA\u4EC0\u9EBC\u8981\u6539\u9019\u500B\u65E5\u671F...",value:reasons[phaseKey]||'',onChange:e=>setReasons({...reasons,[phaseKey]:e.target.value})}));// 開／關兩態的小按鈕（排序選項用）。full=true 是放在下拉面板裡的整寬版本
...
const[activeView,setActiveView]=useState(()=>urlOne('view',['table','dashboard'],'table'));const[expandedRows,setExpandedRows]=useState(new Set());// 最多一個元素（見 toggleRow）；每個改排序的入口都要先呼叫這支
...
const[showSaveErrors,setShowSaveErrors]=useState(false);// ─── 時程異動稽核（第 13 批）───
...
const[actor,setActor]=useState({empId:null,source:'unknown',allowSimulation:false});const[isActorModalOpen,setIsActorModalOpen]=useState(false);// 阻擋型提示視窗（NID 重複、必填未完成）——比 toast 更難被忽略
...
const pageWidth=present?'max-w-none':activeView==='table'?'max-w-[1600px]':'max-w-[1440px]';// 工具列下拉面板：同時只開一個（'sort' | 'data' | null）
...
//    重新匯入後那一欄就該自己回來
const hasNotesLink=useMemo(()=>requirementsData.some(it=>(it.notesLink||'').trim()),[requirementsData]);// ⚠️ 'status'（OverallStatus）2026-08-21 曾併進 StatusID 欄，
...
// 否則展開的明細會撐出多餘的空白欄
const colCount=(compact?9:showCol('notesLink')?16:15)-(printing&&showCol('actions')?1:0);// ⚠️ 讀取失敗一定要出聲（2026-08-23 / 第 25 批）。原本是
...
const fetchAssignees=async()=>{try{const res=await fetch(api('/api/assignees'));if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json();setAssigneeList(Array.isArray(data)?data:[]);setAssigneeError('');}catch(err){console.error('Failed to fetch assignees:',err);setAssigneeError('指派人員名單讀取失敗，下拉選單只會顯示這筆目前指到的人。');}};const fileInputRef=React.useRef(null);// 統一的操作回饋，3 秒後自動消失。
...
const TOAST_MS=(message,type)=>{const base=3000+String(message||'').length*90;return Math.min(12000,Math.max(type==='error'?5000:3000,base));};const showToast=(message,type='success')=>{setToast({message,type});if(toastTimer.current)clearTimeout(toastTimer.current);toastTimer.current=setTimeout(()=>setToast(null),TOAST_MS(message,type));};useEffect(()=>()=>{if(toastTimer.current)clearTimeout(toastTimer.current);},[]);const fetchReqs=async()=>{// 首次（或前一次失敗、手上根本沒有資料）才換掉 tbody；之後的重抓只淡化表格。
// ⚠️ 不要退回「一律 setIsLoading(true)」——那會讓每一次儲存都閃一次
// 「資料載入中…」，看起來像整張表被清空了
const first=!loadedOnceRef.current;if(first)setIsLoading(true);else setRefreshing(true);try{const res=await fetch(api('/api/requirements'));if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json();setRequirementsData(Array.isArray(data)?data:[]);setLoadError('');loadedOnceRef.current=true;// 「畫面上這份資料是什麼時候抓的」。⚠️ 只在成功時更新 ——
...
setRequirementsData([]);setLoadError('無法讀取需求資料，請確認後端服務與資料庫連線是否正常。');// 手上已經沒有資料了，下一次重試要走回「首次載入」的完整提示
...
const fetchHistory=async()=>{try{const res=await fetch(api('/api/history'));if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json();const list=Array.isArray(data)?data:[];setHistoryEntries(list);setHistoryError('');return list;}catch(err){console.error('Failed to fetch history:',err);setHistoryEntries([]);// 「查不到軌跡」與「沒有被改過」在畫面上長得一模一樣，一定要講出差別
setHistoryError('時程異動軌跡讀取失敗，畫面上的異動次數（⚠ 與「時程異動」）暫時不是實際數字。');return null;}};// Windows 帳號偵測（作法對齊 C:\Gantt）。
// /api/whoami 需要驗證，非網域環境會回 401 —— 靜默忽略即可，
// 寫入照常進行，稽核紀錄的 ChangedBy 留空而已，不要因此擋住存檔。
const detectActor=async()=>{let allow=false;try{const info=await fetch(api('/api/authinfo'));if(info.ok)allow=!!(await info.json()).allowSimulation;}catch(err){/* 取不到就當不開放模擬 */}try{const res=await fetch(api('/api/whoami'));if(res.ok){const d=await res.json();if(d.empId){setActor({empId:d.empId,source:'windows',allowSimulation:allow});return;}}}catch(err){/* 401 或非網域 → 落到下面 */}setActor({empId:null,source:'unknown',allowSimulation:allow});};useEffect(()=>{fetchReqs();fetchAssignees();fetchHistory();detectActor();},[]);// ─── 手動重新整理（2026-08-24 / 第 27 批）───
...
if(res.status===400||res.status===403){const body=await res.json().catch(()=>({}));setAlertModal({title:res.status===403?'匯入被拒絕':'匯入失敗',message:body.message||`匯入被拒絕 (HTTP ${res.status})`});return;}if(!res.ok)throw new Error(`HTTP ${res.status}`);const result=await res.json();// ⚠️ 重複 NID 的處理已於 2026-08-23 移除（連同後端回應的 duplicateNids）——
...
// 只是漏在前端這一側
await Promise.all([fetchReqs(),fetchHistory()]);}catch(err){console.error(err);showToast('匯入失敗：'+err.message,'error');}})});return;// 後續邏輯移到 onConfirm
...
const isPhaseOpen=phaseKey=>{const gate=PHASES[phaseKey]?.gate;if(!gate)return true;// ① 永遠開放
const gp=PHASES[gate];const vals=editingData?.[gp.obj]||{};return isValidVal(vals[gp.endKey]);};const gateHint=phaseKey=>{const gate=PHASES[phaseKey]?.gate;return gate?`請先完成 ${PHASES[gate].label} 的日期`:'';};// 以下的 helper 一律經由 PHASES 查表 —— ② MSD 確認 與 ③ MSD 開發 的日期
...
const isPhaseModified=phaseKey=>{if(!editingData?.id)return false;const ph=PHASES[phaseKey];const original=requirementsData.find(d=>d.id===editingData.id);if(!original)return false;const oldP=original[ph.obj]||{};const newP=editingData[ph.obj]||{};return ph.fields.some(f=>(oldP[f]||'')!==(newP[f]||''));};// **End 有沒有被改掉**（② 的 End 就是 confirm）。這才是「日期異動」的定義 ——
// 2026-08-22 使用者定調：改 End 才算異動、要填理由；改 Start 沒關係。
// ⚠️ 首次填寫（原本是空的）一樣不算異動，與既有規則一致
const isPhaseEndModified=phaseKey=>{if(!editingData?.id)return false;const ph=PHASES[phaseKey];const original=requirementsData.find(d=>d.id===editingData.id);if(!original)return false;const oldEnd=(original[ph.obj]||{})[ph.endKey]||'';const newEnd=(editingData[ph.obj]||{})[ph.endKey]||'';return!!oldEnd&&oldEnd!==newEnd;};// ─── 階段完成 Done（第 15 批）───
...
const phaseDoneEntry=phaseKey=>{const all=editingData?.id?historyMap.get(editingData.id)||[]:[];const lastRollbackId=all.reduce((max,h)=>(h.changeType==='規格回退'||h.changeType==='撤銷完成')&&h.phase===phaseKey&&h.id>max?h.id:max,0);return[...all].reverse().find(h=>h.phase===phaseKey&&(h.changeType==='提早完成'||h.changeType==='延期完成')&&h.id>lastRollbackId);};// 最後一筆有效的完成紀錄（跨階段取 id 最大）—— 只有它旁邊會出現「撤銷」（第 66 批）。
// LIFO：主要階段與「一併記錄」的階段各是一筆、主要階段寫在最後，所以第一次撤銷的
// 一定是使用者真的按下去的那一個。後端 /undo-done 用同一條 SQL 挑，兩邊看同一筆
const latestDoneEntry=()=>{let best=null;for(const k of PHASE_KEYS){const e=phaseDoneEntry(k);if(e&&(!best||e.id>best.id))best=e;}return best;};// opts.backfill = 事後補記（第 70 批）：階段早就走過了、只補一筆完成紀錄，StatusID 不動。
// 差別只在：沒有「一併記錄」那段、上限多一道 backfillMax、預設日期是原訂日（那個階段多半是很久以前的事，
// 預設今天幾乎必然是一次假延期）、送出時帶 backfill:true
const handleDone=(phaseKey,opts)=>{const backfill=!!opts?.backfill;const ph=PHASES[phaseKey];const original=requirementsData.find(d=>d.id===editingData?.id);const planned=original?.[ph.obj]?.[ph.endKey];if(!isDateVal(planned)){setAlertModal({title:'尚未壓日期',message:`「${ph.label}」還沒有${phaseKey==='confirm'?'確認日期':'結束日期'}。\n\n請先填寫並儲存，再標記完成。`});return;}// 視窗裡改了日期卻還沒存，按完成會拿舊值去比對，結果與畫面對不起來
if(isPhaseModified(phaseKey)){setAlertModal({title:'有尚未儲存的日期異動',message:`「${ph.label}」的日期在這個視窗裡被改過但還沒儲存。\n\n請先儲存變更，再標記完成。`});return;}// A7：**任何**還沒儲存的欄位都要先擋（不只是這個階段的日期）。
// 標記完成成功後視窗會關掉並重新載入，剛打的現況描述、MP Saving、負責人
// 全部會被靜靜丟掉 —— 使用者不會知道，因為畫面上只看到「已標記完成」的成功訊息
if(isEditDirty()){setAlertModal({title:'有尚未儲存的變更',message:'這個視窗裡還有其他沒儲存的欄位（例如現況描述、負責人）。\n\n'+'標記完成會重新載入這筆資料，那些變更會遺失。\n\n請先按「儲存變更」，再回來標記完成。'});return;}// ─── 完成日可選（第 58 批，2026-09-10 使用者要求）───
...
date:original[p.obj][p.endKey],checked:true};});const cap=backfill?backfillMax(original,phaseKey):{max:TODAY_ISO,label:'',actual:false};setDoneModal({phaseKey,label:ph.label,planned,plannedStart,dateLabel:phaseKey==='confirm'?'確認日':'結束日',doneStage:ph.doneStage,prevKey:prev?.key||'',prevLabel:prev?.label||'',prevEnd:prev?.end||'',prevActual:!!prev?.actual,max:cap.max,capLabel:cap.label,capActual:cap.actual,// 預設今天：多數情況仍然是當天就來按。補記則預設原訂日（準時、不計次），由使用者改成實際那一天
date:backfill?planned>cap.max?cap.max:planned:TODAY_ISO,backfill,curStage,extras});};// ─── 主要階段的完成日下限（第 61 批，2026-09-10）───
...
const submitDone=()=>{const m=doneModal;if(!m)return;if(!isDateVal(m.date)){setAlertModal({title:'請選擇完成日',message:'完成日必須是有效的日期（YYYY-MM-DD）。'});return;}// 下限隨「一併記錄」的勾選即時變動（第 61 批），所以這裡重算、不讀 state
const mainMin=doneMainMin(m).min;if(m.date>m.max||m.date<mainMin){setAlertModal({title:'完成日超出可選範圍',message:`可以選的範圍是 ${mainMin} ~ ${m.max}。\n\n目前選的是 ${m.date}。`});return;}if(!doneExtrasOk(m)){setAlertModal({title:'一併記錄的完成日超出範圍',message:'有階段的完成日不在可選範圍內。\n\n'+'每一個階段都不可能比前一階段更早完成，也不可能比後一階段更晚完成。'});return;}runExclusive(async()=>{try{const res=await fetch(api(`/api/requirements/${editingData.id}/done`),{method:'POST',headers:{'Content-Type':'application/json'},// ⚠️ alsoComplete 只帶**視窗上勾起來**的那幾筆（第 60 批）。
// 後端每一筆都會自己再驗一次範圍與「是不是已經完成過」
body:JSON.stringify({phase:m.phaseKey,completedAt:m.date,// 事後補記（第 70 批）：後端據此跳過「已經走過」的 guard、不動 StatusID
backfill:!!m.backfill,alsoComplete:(m.extras||[]).filter(e=>e.checked).map(e=>({phase:e.phaseKey,completedAt:e.date})),actorEmpId:actor.empId||'',actorSource:actor.source})});const bodyJson=await res.json().catch(()=>({}));if(!res.ok){setAlertModal({title:m.backfill?'無法補記完成':'無法標記完成',message:bodyJson.message||`HTTP ${res.status}`});return;}setDoneModal(null);setEditingData(null);setIsModalOpen(false);await Promise.all([fetchReqs(),fetchHistory()]);showToast(bodyJson.message||'已標記完成');}catch(err){console.error(err);showToast('標記完成失敗：'+err.message,'error');}});};// 階段標題旁要顯示什麼：已完成 → 結果標籤；還沒完成且已壓日期 → 完成鈕；
// 連日期都還沒壓 → 什麼都不顯示（沒有原訂日就沒有提早／延期可言）
const donePanel=phaseKey=>{if(!editingData?.id)return null;const ph=PHASES[phaseKey];const done=phaseDoneEntry(phaseKey);if(done){// ⚠️ 走 changeTypeStyle()（2026-08-23 / 第 23 批補上）——
...
const handleUndoDone=(phaseKey,done)=>{if(isEditDirty()){setAlertModal({title:'有尚未儲存的變更',message:'這個視窗裡還有沒儲存的欄位。\n\n撤銷完成會重新載入這筆資料，那些變更會遺失。\n\n請先按「儲存變更」，再回來撤銷。'});return;}const original=requirementsData.find(d=>d.id===editingData?.id);// ─── 還原 End 會不會抬到下一階段的 End 之後（第 67 批，2026-09-11）───
...
await Promise.all([fetchReqs(),fetchHistory()]);showToast(bodyJson.message||'已撤銷');}catch(err){console.error(err);showToast('撤銷失敗：'+err.message,'error');}});};// ─── 規格回退（第 16 批）───
...
const requiredFieldsFor=orig=>[{key:'nid',label:'NID',get:d=>d.nid},{key:'mainCat',label:'Main Cat',get:d=>d.mainCat},{key:'subCat',label:'Sub Cat',get:d=>d.subCat},{key:'emsOwner',label:'EMS 負責人',get:d=>d.emsOwner},// ⚠️ 開始日**不再是必填**（2026-08-22 使用者定調：Start 不重要，
...
const validateEdit=()=>{const fields={},groups=[];if(!editingData)return{fields,groups};const mark=(k,msg)=>{if(k&&!fields[k])fields[k]=msg;};// 這筆資料已儲存的值。必填、跨階段順序、gating 都要跟它比對
const saved=editingData.id?requirementsData.find(d=>d.id===editingData.id):null;// 必填欄位
const missing=requiredFieldsFor(saved).filter(f=>!String(f.get(editingData)||'').trim());if(missing.length>0){missing.forEach(f=>mark(f.key,'必填'));groups.push({title:'必填欄位未完成',items:missing.map(f=>f.label)});}// 每個區間的結束日不可早於開始日。日期是 "YYYY-MM-DD"，字串比較即等於時間比較
const badRanges=['spec','msd','uat'].map(k=>({k,label:PHASES[k].label,obj:PHASES[k].obj,p:editingData[PHASES[k].obj]||{}})).filter(({p})=>p.start&&p.end&&p.start>p.end);if(badRanges.length>0){badRanges.forEach(({obj})=>{mark(`${obj}.start`,'開始日晚於結束日');mark(`${obj}.end`,'結束日早於開始日');});groups.push({title:'日期區間不合理（End Date 早於 Start Date）',items:badRanges.map(({label})=>label)});}// ─── 跨階段的 End 必須遞增（2026-08-22 / 第 21 批）───
...
const orderChain=[{label:'1_EMS規格確認 結束日',obj:'spec',field:'end'},{label:'2_MSD確認中 確認日',obj:'msd',field:'confirm'},{label:'3_MSD開發中 結束日',obj:'msd',field:'end'},{label:'4_EMS驗收 結束日',obj:'uat',field:'end'}].map(x=>({...x,now:(editingData[x.obj]||{})[x.field]||'',was:((saved||{})[x.obj]||{})[x.field]||''}));const badOrder=[];for(let i=1;i<orderChain.length;i++){const prev=orderChain[i-1],cur=orderChain[i];if(!isDateVal(prev.now)||!isDateVal(cur.now))continue;if(cur.now>=prev.now)continue;const touched=!saved||prev.now!==prev.was||cur.now!==cur.was;if(touched){mark(`${cur.obj}.${cur.field}`,`不可早於${prev.label} ${prev.now}`);badOrder.push(`${cur.label} ${cur.now} 早於 ${prev.label} ${prev.now}`);}}if(badOrder.length>0){groups.push({title:'階段日期的先後順序不合理（四個階段是依序進行的）',items:badOrder});}// 階段順序 gating（第 14 批）。日期欄本身已經 disable，正常操作走不到這裡，
...
const gateBad=PHASE_KEYS.filter(key=>{if(!PHASES[key].gate||isPhaseOpen(key))return false;const ph=PHASES[key];return!isValidVal(saved?.[ph.obj]?.[ph.endKey])&&isValidVal(editingData?.[ph.obj]?.[ph.endKey]);});if(gateBad.length>0){gateBad.forEach(k=>mark(`${PHASES[k].obj}.${PHASES[k].endKey}`,gateHint(k)));groups.push({title:'階段順序不正確（前置階段還沒填完，不能先壓日期）',items:gateBad.map(k=>`${PHASES[k].label}（${gateHint(k)}）`)});}// ─── 已走完的階段不可清空 End、清空也不可挖洞（第 66 批 H1，2026-09-11）───
...
if(saved){const nNow=parseInt(normStageCode(editingData.stageCode),10)||savedStage(saved);const badClear=[];for(let i=0;i<orderChain.length;i++){const c=orderChain[i];if(!(isDateVal(c.was)&&!isDateVal(c.now)))continue;const stageOf=i+1;const word=c.field==='confirm'?'確認日':'結束日';if(stageOf<nNow){mark(`${c.obj}.${c.field}`,`已走完的階段不可清空${word}，要退回請用規格回退`);badClear.push(`「${PHASES[PHASE_KEYS[i]].label}」已經走完（StatusID = ${STAGE_CODES[String(nNow)]?.label||nNow}），${word}不可清空；要退回這個階段請用「🔄 規格回退」`);continue;}// orderChain 與 PHASE_KEYS 都是 spec → confirm → msd → uat 的順序
...
const nidVal=String(editingData.nid||'').trim();const dup=nidVal&&requirementsData.find(d=>String(d.nid||'').trim()===nidVal&&d.id!==editingData.id);if(dup){mark('nid','這個編號已被使用');groups.push({title:'NID 重複（NID 必須是唯一值）',items:[`NID「${nidVal}」已被「${dup.mainCat||''} / ${dup.subCat||''}」使用`]});}// 解鎖後**改了 End** 才必須留下理由（2026-08-22：改 Start 不算異動）
const noReason=[];for(const key of PHASE_KEYS){if(unlockedSections[key]&&isPhaseEndModified(key)){if(!unlockCategories[key]){mark(`reason.${key}`,`請選擇異動原因分類（${REASON_CATEGORIES.join(' / ')}）`);noReason.push(`${PHASES[key].label}：缺原因分類`);}else if(!unlockReasons[key]||!unlockReasons[key].trim()){mark(`reason.${key}`,'請填寫文字說明');noReason.push(`${PHASES[key].label}：缺文字說明`);}}}if(noReason.length>0){groups.push({title:'日期被修改了，必須填寫異動原因',items:noReason});}// 手動改 StatusID 一定要留原因（第 19 批 / A5）。後端也擋一次。
// Status（OverallStatus）不強制 —— 它是人工壓的旗標，每次都要寫理由太吵；
// 它仍然會被寫進稽核列（後端組的說明文字），只是不必打字
const stageChanged=!!saved&&normStageCode(saved.stageCode)!==normStageCode(editingData.stageCode);if(stageChanged){const toLabel=STAGE_CODES[normStageCode(editingData.stageCode)]?.label||'未設定';const fromN=savedStage(saved),toN=parseInt(normStageCode(editingData.stageCode),10)||0;const lacking=stagePrereqMissing(editingData.stageCode,editingData);// StatusID 不可空白（第 66 批 H3）；只能往前（H2）—— 下拉已經把往回的選項停用，
...
if(lacking.length>0){mark('stage',`不能改成「${toLabel}」，前面的階段還沒填完`);groups.push({title:`StatusID 改成「${toLabel}」代表前面都已走完，但這些階段還缺日期`,items:lacking});}else if(!unlockCategories.stage){mark('reason.stage',`請選擇異動原因分類（${REASON_CATEGORIES.join(' / ')}）`);groups.push({title:'手動調整 StatusID 必須填寫異動原因',items:[`改為「${toLabel}」：缺原因分類`]});}else if(!unlockReasons.stage||!unlockReasons.stage.trim()){mark('reason.stage','請填寫文字說明');groups.push({title:'手動調整 StatusID 必須填寫異動原因',items:[`改為「${toLabel}」：缺文字說明`]});}}// ─── Status = Done ⇔ StatusID = 5（第 67 批，2026-09-11）───
...
const errOf=k=>showSaveErrors?editProblems.fields[k]||'':'';const errBorder=k=>errOf(k)?'var(--tone-alert)':'var(--border-table)';const handleSave=async e=>{if(e)e.preventDefault();// ─── 驗證一次算完（見 validateEdit）───
// 全部問題一起列出，並在對應欄位就地標紅。
// ⚠️ 不要退回「一段一個 return」—— 那會變成缺三個必填就要按三次儲存、
//    看三次彈窗，而且關掉彈窗之後畫面上沒有任何一格是紅的，
//    使用者得自己回想剛剛那句話講的是哪一欄
if(editProblems.groups.length>0){setShowSaveErrors(true);const g=editProblems.groups;setAlertModal({title:g.length===1?g[0].title:`有 ${g.length} 類問題需要修正`,message:(g.length===1?g[0].items.map(i=>'・'+i).join('\n'):g.map(x=>`【${x.title}】\n`+x.items.map(i=>'・'+i).join('\n')).join('\n\n'))+'\n\n有問題的欄位已在編輯視窗中標紅，改好之後紅字會自己消失。'});return;}setShowSaveErrors(false);// 這筆資料已儲存的值。下面組稽核用的 changeMeta 要跟它比對
...
if(stageChanged||statusChanged){changeMeta.stage={category:unlockCategories.stage||'',note:unlockReasons.stage||''};}// 送出前把空白的 Start 補成 End（後端也會做一次，兩邊同一套規則）
let payload={...applyStartDefaults(editingData),changeMeta,actorEmpId:actor.empId||'',actorSource:actor.source};const method=payload.id?'PUT':'POST';const url=api('/api/requirements')+(payload.id?'/'+payload.id:'');// ⚠️ 包在 runExclusive 裡（第 26 批）：連點兩下「確認新增」會送出兩筆，
...
if(body.conflict)fetchReqs();return;}if(!res.ok)throw new Error(`HTTP ${res.status}`);setEditingData(null);setIsModalOpen(false);const[list,hist]=await Promise.all([fetchReqs(),fetchHistory()]);showToast(payload.id?'已儲存變更':'已新增需求');// ─── 存完之後，當前階段沒壓日期就問要不要通知下一棒（第 39 批）───
...
//    最壞情況是多問一次，不會少問。
if(fresh&&unsetDuePhase(fresh)){const preview=notifyPreview(fresh);if(preview&&!preview.problem&&!phaseNotifiedEntry(fresh.id,preview.phase.key,preview.toEmail,hist))askNotifyUnset(fresh);}}catch(err){console.error(err);showToast('儲存失敗：'+err.message,'error');}});};const handleDelete=async item=>{// 軟刪除：改用 confirmModal 取代原生 confirm()，避免在工廠 PC 被安全設定封鎖。
...
//（與匯入的 A8 是同一類問題）
await Promise.all([fetchReqs(),fetchHistory()]);showToast('已刪除');}catch(err){console.error(err);showToast('刪除失敗：'+err.message,'error');}})});};// ─── 未儲存變更的判定（2026-08-22）───
// 開視窗時存一份快照，關視窗前比對。editingData 一律用 spread 更新
// （鍵的順序不變），所以 JSON 字串比對就夠用，不必逐欄位寫比較。
// 改回原值再關不會跳提示 —— 那本來就沒有變更
const editSnapshot=React.useRef('');const isEditDirty=()=>!!editingData&&JSON.stringify(editingData)!==editSnapshot.current;// 關閉編輯視窗。有未儲存的變更就先問一次 ——
...
const focusPhaseRef=React.useRef(null);const openEdit=(item,phaseKey=null)=>{focusPhaseRef.current=PHASES[phaseKey]?phaseKey:null;setEditingData(item);editSnapshot.current=JSON.stringify(item);setUnlockedSections({spec:false,confirm:false,msd:false,uat:false});setUnlockReasons({spec:'',confirm:'',msd:'',uat:'',stage:''});setUnlockCategories({spec:'',confirm:'',msd:'',uat:'',stage:''});setStageUnlocked(false);setShowSaveErrors(false);setIsModalOpen(true);};const openAdd=()=>{const today=new Date();const currentYM=today.getFullYear()+'/'+String(today.getMonth()+1).padStart(2,'0');const todayIso=`${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`;// 自動產生的預設值：OverallStatus=Init、StatusID=1、RegDate=今天（YearMonth 由後端從 RegDate 反推）
const blank={isNew:true,nid:'',regDate:todayIso,yearMonth:currentYM,mainCat:'',subCat:'',status:'Init',stageCode:'1',remark:'',notesLink:'',emsOwner:'',msdOwner:'',currentStatus:'',mpSaving:'',spec:{start:'',end:'',history:''},msd:{confirm:'',confirmNote:'',confirmHistory:'',start:'',end:'',history:''},uat:{start:'',end:'',history:''}};setEditingData(blank);editSnapshot.current=JSON.stringify(blank);setUnlockedSections({spec:false,confirm:false,msd:false,uat:false});setUnlockReasons({spec:'',confirm:'',msd:'',uat:'',stage:''});setUnlockCategories({spec:'',confirm:'',msd:'',uat:'',stage:''});setStageUnlocked(false);setShowSaveErrors(false);setIsModalOpen(true);};// ─── Esc 關閉視窗（2026-08-22）───
...
const openModalCount=[isAssigneeModalOpen,!!editingData,isActorModalOpen,!!alertModal,!!rollbackModal,!!confirmModal,!!doneModal,!!undoModal,!!histModal].filter(Boolean).length;const topModalEl=()=>{const all=document.querySelectorAll('[data-ct-modal]');return all.length?all[all.length-1]:null;};// ⚠️ 「開窗前的焦點」不可以在視窗開起來之後才讀 `document.activeElement`（實測過）：
...
const el=document.querySelector(`[data-ct-focus="${key}"]`);if(!el)return;// 先捲到定位再聚焦。gate 沒過的欄位是 disabled（focus 無效），
// 但捲過去仍然有意義 —— 那顆鎖旁邊就寫著「要先填完前一階段」
try{el.scrollIntoView({block:'center'});}catch(e){/* noop */}if(!el.disabled){try{el.focus();}catch(e){/* noop */}}},[openModalCount]);useEffect(()=>{if(openModalCount===0)return;const onKey=e=>{if(e.key!=='Tab')return;const top=topModalEl();if(!top)return;const items=Array.from(top.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])')).filter(el=>el.offsetWidth>0||el.offsetHeight>0);if(!items.length){e.preventDefault();top.focus();return;}const first=items[0],last=items[items.length-1];const cur=document.activeElement;if(!top.contains(cur)){e.preventDefault();(e.shiftKey?last:first).focus();return;}if(!e.shiftKey&&cur===last){e.preventDefault();first.focus();}else if(e.shiftKey&&cur===first){e.preventDefault();last.focus();}};// capture：要在元素自己的 keydown 之前決定要不要攔下來
...
if(note.includes('未確認送出'))return false;const m=note.match(NOTIFY_TO_RE);return!!m&&m[1].trim().toLowerCase()===want;})||null;};const notifyPreview=item=>{const ph=unsetDuePhase(item);if(!ph)return null;const side=ph.side;// 這一階段該由誰壓日期
...
useEffect(()=>{const p=new URLSearchParams();if(activeView!=='table')p.set('view',activeView);if(searchQuery)p.set('q',searchQuery);if(stageFilter.length)p.set('stage',stageFilter.join(','));if(emsFilter!=='All')p.set('ems',emsFilter);if(msdFilter!=='All')p.set('msd',msdFilter);if(dueFilter!=='All')p.set('due',dueFilter);// ⚠️ `prog` 與 `dp` 一樣**兩個方向都要寫**（第 49 批）：它的預設值是
...
const err=await res.json().catch(()=>null);showToast(err?.message||`新增失敗 (HTTP ${res.status})`,'error');}}catch(err){console.error(err);showToast('新增失敗：'+err.message,'error');}};// 停用不是刪除：既有需求指到的人被刪掉，那筆指派就查不到對應的人了。
...
const shownHist=rowHist.filter(isMeaningfulEntry);const hasHist=shownHist.length>0;// 各階段的逾期／即將到期狀態，整列取最嚴重的那個當左側色條。
...
const notifyEntries=shownHist.filter(h=>h.changeType==='通知寄送');const changeEntries=shownHist.filter(h=>h.changeType!=='init'&&h.changeType!=='通知寄送');const initEntries=shownHist.filter(h=>h.changeType==='init');// 摘要行要用的資料。⚠️ 收件者從 Note 解析（後端格式見 Program.cs 的 auditNote）——
