...
function App() {
  ...
  const toggleNotifyOpen = id => setNotifyOpen(m => ({
    ...m,
    [id]: !m[id]
  }));
  ...
  const toggleDuePriority = next => {
    setDuePriority(next);
    try {
      localStorage.setItem('ct.duePriority', next ? '1' : '0');
    } catch (e) {/* 鎖了就算了 */}
  };
  ...
  const [legendOpen, setLegendOpen] = useState(() => {
    try {
      return localStorage.getItem('ct.legendOpen') === '1';
    } catch (e) {
      return false;
    }
  });
  const toggleLegend = () => setLegendOpen(v => {
    const next = !v;
    try {
      localStorage.setItem('ct.legendOpen', next ? '1' : '0');
    } catch (e) {/* 鎖了就算了 */}
    return next;
  });
  ...
  useEffect(() => {
    const measure = () => {
      ...
      setHeadOffsets(prev => prev.group === group && prev.col === col ? prev : {
        group,
        col
      });
  ...
  const specEndRequired = !!(editingData?.id && isDateVal(requirementsData.find(d => d.id === editingData.id)?.spec?.end));
    ...
    await runExclusive(async () => {
      try {
        ...
        if (fresh && unsetDuePhase(fresh)) {
          ...
          if (preview && !preview.problem && !phaseNotifiedEntry(fresh.id, preview.phase.key, preview.toEmail, hist)) askNotifyUnset(fresh);
        }
      } catch (err) {
        console.error(err);
  ...
  const phaseNotifiedEntry = (itemId, phaseKey, toEmail, entries) => {
    ...
    const all = entries ? entries.filter(h => h.requirementId === itemId) : historyMap.get(itemId) || [];
    const lastRollbackId = all.reduce((max, h) => h.changeType === '規格回退' && h.phase === phaseKey && h.id > max ? h.id : max, 0);
  ...
  const askNotifyUnset = item => {
    ...
    const who = [item.nid && `NID ${item.nid}`, item.mainCat, item.subCat].filter(Boolean).join(' / ');
    const ccLine = p.ccEmail ? `副本　：${p.ccName} <${p.ccEmail}>` : p.ccBadFormat ? `副本　：${p.ccName}（信箱格式不正確，這次不會有副本）` : p.ccName ? `副本　：${p.ccName}（指派人員主檔裡沒有信箱，這次不會有副本）` : `副本　：（${p.ccDept} 還沒指派負責人，這次不會有副本）`;
    ...
    const againLine = prev ? `⚠️ 這個階段已經在 ${prev.changedAt} 通知過 ${p.toName} 了，這會是第二封。\n\n` : '';
    setConfirmModal({
      title: '寄信通知壓定日期',
      message: `需求「${who}」目前已經走到「${p.phase.label}」，但這個階段還沒有壓日期。\n\n` + `要寄信通知 ${p.side} 負責人進系統把日期壓上去嗎？\n\n` + `${fromLine}\n收件者：${p.toName} <${p.toEmail}>\n${ccLine}\n\n` + againLine + '按「確定」會立刻寄出，並在這筆需求的時程變更軌跡留下一筆「通知寄送」紀錄。',
      onConfirm: () => runExclusive(async () => {
        ...
        try {
          ...
          if (b.queued) {
            ...
            if (b.ccMissing) setAlertModal({
              title: '已寄出，但沒有副本',
              message: `通知已寄給 ${b.toName} <${b.to}>。\n\n` + '⚠️ ' + (b.ccReason || `副本 ${b.ccName} 在指派人員主檔裡沒有填 EMAIL，這一封沒有副本給他。`)
            });
          }
        } catch (err) {
          ...
          if (err.name === 'AbortError') setAlertModal({
            title: '等太久，已停止等待',
  ...
  const stageFacets = useMemo(() => buildStageFacets(true), [requirementsData, searchQuery, emsFilter, msdFilter, dueFilter, alertFilter, colFilters, dueInfo, changedIdSet]);
  // 同一份數字但**含**進度篩選。只用來判斷「這一階段是不是整群被進度篩選擋住了」——
  // 是的話點它就順手把進度篩選解除（否則使用者會點到一個必然 0 筆的組合）
  const stageFacetsUnderProgress = useMemo(() => buildStageFacets(false), [requirementsData, searchQuery, emsFilter, msdFilter, dueFilter, alertFilter, progressFilter, colFilters, dueInfo, changedIdSet]);
  ...
  const hasNonDefaultFilter = searchTerm || stageFilter.length > 0 || emsFilter !== 'All' || msdFilter !== 'All' || dueFilter !== 'All' || alertFilter !== 'All' || progressFilter === 'done' || colFilterCount > 0;
  ...
  const searchBlockedCount = useMemo(() => {
    ...
    return requirementsData.filter(it => matchExceptStage(it, true) && (stageFilter.length === 0 || stageFilter.includes(effStageCode(it))) && !matchProgressFilter(it, progressFilter)).length;
  ...
  const activeChips = useMemo(() => {
    ...
    if (dueFilter !== 'All') out.push({
      ...
      onRemove: () => {
        setDueFilter('All');
        if (dueFilter === 'attention') setDuePriority(readDuePriorityPref());
      }
    });
  ...
  const renderChip = c => /*#__PURE__*/React.createElement("span", {
    key: c.id,
  ...
  const renderClipWarning = floating => {
    ...
    const tip = atMinZoom ? `${head}。\n投影倍率已經是最低、而且已經是精簡模式了，請把瀏覽器視窗拉寬（或改用解析度更高的輸出）` : `${head}。\n點一下降一級投影倍率`;
    const act = atMinZoom ? undefined : () => stepZoom(-1);
    return /*#__PURE__*/React.createElement("button", {
      onClick: act,
      disabled: !act,
      className: floating ? 'ctl-sm fixed bottom-6 right-6 z-[70] no-print shadow-2xl disabled:cursor-default present-zoom' : 'ctl-sm flex-shrink-0 disabled:cursor-default',
      style: floating
      ...
      "aria-label": act ? `布幕右邊有 ${clipPx} 像素在畫面外，點擊降低投影倍率` : `布幕右邊有 ${clipPx} 像素在畫面外，請把視窗拉寬`,
      title: tip
    }, "\u26A0 \u53F3\u908A\u88AB\u5207\u6389");
  };
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `min-h-screen page-shell${present ? ' present' : ''}`,
    style: {
      color: 'var(--text-secondary)',
    ...
    "aria-label": "\u95DC\u9589\u9019\u5247\u8A0A\u606F",
    title: "\u95DC\u9589"
  }, "\u2715")), scrolledX && renderClipWarning(true), /*#__PURE__*/React.createElement("header", {
    ref: appHeaderRef,
  ...
  }, "\uD83D\uDDA5\uFE0F ", actor.empId || '未識別', actor.source === 'simulated' && ' (模擬)'), !scrolledX && renderClipWarning(false), present && /*#__PURE__*/React.createElement("div", {
  ...
  React.createElement("div", {
  ...
  }, "\u5340\u9593 ", trendView.rows[0].name.replace('20', ''), " \u2013 ", trendView.rows[trendView.rows.length - 1].name.replace('20', ''), "\uFF0E\u5171 ", trendView.rows.reduce((s, r) => s + r.ongoing + r.done, 0), " \u4EF6"))), /*#__PURE__*/React.createElement("div", {
    className: "t-card p-5"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between gap-2 mb-4 flex-wrap"
  }, /*#__PURE__*/React.createElement("h2", {
  ...
  }, o.count)))))))), activeView === 'table' && /*#__PURE__*/React.createElement("div", {
    className: "space-y-4"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }), hasNonDefaultFilter && /*#__PURE__*/React.createElement("button", {
  ...
  }, "\u4E0B\u8F09", /*#__PURE__*/React.createElement("b", null, "\u5168\u90E8 ", requirementsData.length, " \u7B46"), "\u6210 .xlsx", sortedData.length !== requirementsData.length && /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--tone-warn)'
    }
  }, "\uFF08\u4E0D\u5957\u7528\u756B\u9762\u4E0A\u7684\u7BE9\u9078\uFF0C\u756B\u9762\u76EE\u524D\u662F ", sortedData.length, " \u7B46\uFF09"))), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setOpenMenu(null);
    ...
    return /*#__PURE__*/React.createElement(Fragment, {
      ...
      onClick: () => {
        ...
        if (picking && progressFilter !== 'All' && (stageFacetsUnderProgress[o.k] ?? 0) === 0 && (stageFacets[o.k] ?? 0) > 0) setProgressFilter('All');
        setStageFilter(prev => picking ? [...prev, o.k] : prev.filter(x => x !== o.k));
      },
      className: "ctl gap-2",
      style: active ? activeStyle : undefined,
      title: isAll ? '顯示全部（清除已選取的階段，並取消「只看進行中」）' : `StatusID ${o.k} ${o.label}（可複選，再點一次取消）` + (progressFilter !== 'All' && (stageFacetsUnderProgress[o.k] ?? 0) === 0 && (stageFacets[o.k] ?? 0) > 0 ? `。目前的「${PROG_FILTER_LABEL[progressFilter]}」會把這一階段整群擋掉，點下去會一併取消它` : '')
  ...
  }, sortedData.length), " / ", requirementsData.length, " \u7B46"), /*#__PURE__*/React.createElement("span", {
    ...
    title: historyError ? '軌跡讀取失敗的訊息就在圖例列裡，所以現在不能收起來' : '圖例：左側色條、⚠ 未壓日期、⏰ 延期完成、🔄 規格回退…\n點一下顯示／收起。收起只作用在螢幕上，列印時一定會印出來'
  }, /*#__PURE__*/React.createElement("svg", {
    width: "14",
    height: "14",
    viewBox: "0 0 24 24",
    ...
    className: `t-card px-4 py-2.5 flex flex-wrap items-center gap-2${chipsInline ? ' chips-print' : ''}`
  }, /*#__PURE__*/React.createElement("span", {
  ...
  }, "\u751F\u6548\u4E2D\u7684\u689D\u4EF6"), activeChips.map(renderChip), hiddenChipCount > 0 && /*#__PURE__*/React.createElement("span", {
    className: "text-[10px]",
    style: {
      color: 'var(--tone-alert)'
    }
  }, "\u26A0 \u6709 ", hiddenChipCount, " \u500B\u689D\u4EF6\u7684\u6B04\u4F4D\u5728\u76EE\u524D\u6A21\u5F0F\u4E0B\u662F\u6536\u8D77\u4F86\u7684\uFF0C\u4F46\u5B83\u4ECD\u5728\u904E\u6FFE"), searchBlockedCount > 0 && /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1.5 no-print"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-[11px]",
    style: {
      color: 'var(--text-tertiary)'
    }
  }, "\u53E6\u6709 ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: 'var(--text-primary)'
    }
  }, searchBlockedCount), " \u7B46", progressFilter === 'ongoing' ? '已完成' : '進行中', "\u7684\u9700\u6C42\u7B26\u5408\u300C", searchTerm, "\u300D"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setProgressFilter('All'),
    className: "ctl-sm",
    title: "\u53D6\u6D88\u300C\u9032\u5EA6\u300D\u9019\u4E00\u689D\uFF0C\u628A\u5B83\u5011\u4E00\u8D77\u986F\u793A\u51FA\u4F86"
  }, "\u4E00\u4F75\u986F\u793A"))), /*#__PURE__*/React.createElement("div", {
    ...
    className: `legend-strip flex flex-wrap items-center gap-x-4 gap-y-1 px-4 py-2.5 text-[10px] rounded-t-[10px]${legendShown ? '' : ' is-collapsed'}`,
    style: {
      color: 'var(--text-muted)',
      background: 'var(--bg-input)',
      borderBottom: '1px solid var(--border-card)'
  ...
  React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
    ...
    return /*#__PURE__*/React.createElement(Fragment, {
      ...
      onNotify: notifyThis,
      onSetDate: () => openEdit(item, 'confirm')
    }), scheduleCell({
      val: item.msd?.end,
      alert: msdAlert,
    ...
    }, histCount, " \u6B21"), notifyEntries.length > 0 && /*#__PURE__*/React.createElement("span", {
      className: "text-[10px] font-bold px-1.5 py-0.5 rounded cursor-help",
      style: {
        color: changeTypeStyle('通知寄送').color,
    ...
    }, "\u2709 \u5DF2\u901A\u77E5 ", notifyEntries.length, " \u6B21")), notifyEntries.length > 0 && /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u5DF2\u901A\u77E5 ", notifyEntries.length, " \u6B21"), lastNotify && /*#__PURE__*/React.createElement("span", {
      className: "truncate"
    }, "\xB7 \u6700\u5F8C ", lastNotify.changedAt, lastNotify.changedBy ? ` · ${lastNotify.changedBy}` : '', notifyToOf(lastNotify) ? ` → ${notifyToOf(lastNotify)}` : ''), /*#__PURE__*/React.createElement("span", {
      className: "ml-auto shrink-0 font-bold",
      style: {
        color: changeTypeStyle('通知寄送').color
      }
    }, notifyOpen[item.id] ? '收合 ▲' : '展開 ▼')), notifyOpen[item.id] && /*#__PURE__*/React.createElement("div", {
      className: "px-2 pb-1.5 max-h-32 overflow-y-auto scrollbar-thin",
      style: {
        borderTop: '1px solid var(--bg-input-border)'
      }
    }, [...notifyEntries].reverse().map(h => /*#__PURE__*/React.createElement("div", {
      key: h.id,
      className: "flex items-center gap-1.5 py-1 text-[11px] leading-tight",
      title: h.note || '',
      style: {
        color: 'var(--text-tertiary)'
      }
      ...
      title: "\u9019\u7B46\u662F\u7528\u6A21\u64EC\u5E33\u865F\u5BEB\u5165\u7684"
    }, "\uFF08\u6A21\u64EC\uFF09")), /*#__PURE__*/React.createElement("span", null, "\u2192 ", notifyToOf(h) || '（收件者不明）'), /*#__PURE__*/React.createElement("span", {
      className: "shrink-0",
      style: {
        color: (PHASES[h.phase] || {}).color || 'var(--text-muted)'
      }
    }, timelineLabelOf(h.phase)), notifyUnsure(h) && /*#__PURE__*/React.createElement("span", {
      className: "shrink-0 font-bold",
      style: {
        color: 'var(--tone-alert)'
      },
      title: "\u9019\u4E00\u5C01\u4EA4\u7D66\u90F5\u4EF6\u7CFB\u7D71\u4E86\uFF0C\u4F46\u6C92\u6709\u78BA\u8A8D\u5230\u771F\u7684\u9001\u51FA\u53BB"
    }, "\u26A0 \u672A\u78BA\u8A8D\u9001\u51FA"))))), !hasTimeline
    ...
    }, historyError ? '軌跡讀取失敗，這不代表沒有變更' : notifyEntries.length > 0 ? '沒有時程變更紀錄（上方是通知紀錄）' : '無變更紀錄') : /*#__PURE__*/React.createElement("div", {
  ...
  }, "End Date ", specEndRequired ? /*#__PURE__*/React.createElement("span", {
    className: "text-red-500"
  }, "*") : /*#__PURE__*/React.createElement("span", {
    className: "font-normal",
    style: {
    ...
    msg: errOf('spec.end')
  }), !specEndRequired && !isDateVal(editingData.spec?.end) && /*#__PURE__*/React.createElement("div", {
    className: "mt-1 text-[11px] leading-relaxed",
    style: {
      color: 'var(--text-muted)'
    }
  }, "\u7559\u7A7A\u7684\u8A71\u9019\u7B46\u6703\u6A19\u6210\u300C\u26A0 \u672A\u58D3\u65E5\u671F\u300D\uFF0C\u5B58\u6A94\u5F8C\u6703\u554F\u4F60\u8981\u4E0D\u8981\u5BC4\u4FE1\u901A\u77E5 EMS \u8CA0\u8CAC\u4EBA\u3002"))), unlockedSections.spec && isPhaseEndModified('spec') && /*#__PURE__*/React.createElement("div", {
    ...
    className: "mt-1"
  }, "\u6E05\u7A7A\u5F8C \u2460 \u6703\u8B8A\u6210\u300C\u26A0 \u672A\u58D3\u65E5\u671F\u300D\uFF0C\u8CC7\u6599\u5217\u4E0A\u6703\u51FA\u73FE\u7D05\u8272\u5FBD\u7AE0\u8207 \u2709 \u901A\u77E5\u9215\u3002\u9019\u7B46\u9700\u6C42\u4ECD\u7136\u53EF\u4EE5\u6B63\u5E38\u5132\u5B58\uFF0C\u91CD\u65B0\u58D3\u65E5\u671F\u6642\u4E5F\u4E0D\u5FC5\u586B\u7570\u52D5\u7406\u7531\uFF08\u6703\u8A18\u6210\u300C\u91CD\u65B0\u6392\u7A0B\u300D\uFF09\u3002")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    className: "block text-xs font-bold mb-1",
    style: {
      color: 'var(--text-secondary)'
    }
